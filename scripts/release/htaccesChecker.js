...
if (!fs.existsSync('./grid-packages/ag-grid-docs/documentation/public')) {
    console.error(
        "./grid-packages/ag-grid-docs/documentation/public doesn't exist - has the documentation build & package been run yet?"
    );
    process.exit(1);
}
...
const redirectsTargets = htaccess
    .split('\n')
...
redirectsTargets.forEach((redirectsTarget) => {
    if (
        !fs.existsSync(`./grid-packages/ag-grid-docs/documentation/public/${redirectsTarget}`) &&
        !fs.existsSync(`grid-packages/ag-grid-docs/src/${redirectsTarget}/index.php`)
    ) {
        console.error(
            `Redirect ending with ${redirectsTarget} - this directory doesn't exist under ./grid-packages/ag-grid-docs/documentation/public/ or under /grid-packages/ag-grid-docs/src/`
        );
        errors = 1;
    }
});
