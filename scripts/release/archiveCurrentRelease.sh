...
ssh -i $SSH_LOCATION -p 2022 ceolter@ag-grid.com "if [ -f "$ARCHIVE_FILENAME" ]; then echo \"$ARCHIVE_FILENAME exists - skipping archive\"; else zip -r --exclude=*archive* $ARCHIVE_FILENAME public_html; fi"
