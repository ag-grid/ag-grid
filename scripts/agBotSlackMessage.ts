...
const args = yargs(hideBin(process.argv))
    .usage(
        'Usage: $0 [package path] --auth-token [auth-token] -grid-channel [grid-channel] --charts-channel [charts-channel] --website-status-channel [website-status-channel] --slack-bot-oauth-token [slack-bot-oauth-token] --debug-channel [debug-channel] --run-context [run-context]'
    )
    .options({
        'auth-token': {
            demandOption: true,
...
(async () => {
    ...
    runContext.status = Object.values(runContext.jobStatuses).every((status) => status === "success") ? "success" : "failure"
