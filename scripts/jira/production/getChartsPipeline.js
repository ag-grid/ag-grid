...
const PIPELINE_TMP_FILE = '/var/www/html/charts/pipeline/pipeline.tmp.json';
const PIPELINE_FILE = '/var/www/html/charts/pipeline/pipeline.json';
const PIPELINE_BAK_FILE = '/var/www/html/charts/pipeline/pipeline.bak.json';

try {
    logger('Charts Pipeline Data Download Started', PIPELINE_LOG);
    const pipelineData = executeJiraRequest(PIPELINE_URL, 'Charts');
    logger('Charts Pipeline Data Downloaded', PIPELINE_LOG);

    saveDataToFile(pipelineData, PIPELINE_TMP_FILE);
    logger('Charts Pipeline Data Saved', PIPELINE_LOG);

    // check we've not downloaded an empty data set (it's very unlikely the pipeline will ever be empty)
    if (pipelineData.length > 1) {
        logger('Charts Updating Existing Pipeline Data', PIPELINE_LOG);
    ...
    logger('Charts Pipeline Data Processing Complete', PIPELINE_LOG);
} catch (e) {
    logger('Error: Charts Pipeline Data Downloaded Failed', PIPELINE_LOG);
