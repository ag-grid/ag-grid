...
export class RangeBarChartProxy extends CartesianChartProxy {
    ...
    protected override getData(params: UpdateParams, axes: AgCartesianAxisOptions[]): any[] {
        // The charts library doesn't perform any statistical analysis within the range bar chart implementation, rather
        // it expects to be given a set of precomputed min/max values, which it renders directly onto the chart.
        // This means that we first need to compute the min/max valuess for each category/series combination, then once we
