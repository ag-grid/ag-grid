...
class MiniBoxPlot extends miniChartWithAxes_1.MiniChartWithAxes {
    constructor(container, fills, strokes, themeTemplateParameters, isCustomTheme) {
        ...
        this.boxPlotGroups = data.map((datum, i) => {
            ...
            const boxPlotGroup = new ag_charts_community_1._Scene.Group();
            const box = new ag_charts_community_1._Scene.Rect();
            const median = new ag_charts_community_1._Scene.Line();
            const topWhisker = new ag_charts_community_1._Scene.Line();
            const bottomWhisker = new ag_charts_community_1._Scene.Line();
            const topCap = new ag_charts_community_1._Scene.Line();
            const bottomCap = new ag_charts_community_1._Scene.Line();
            box.x = left;
            box.y = top;
            box.width = bandwidth;
    ...
    updateColors(fills, strokes, themeTemplateParameters, isCustomTheme) {
        var _a;
        const themeBackgroundColor = themeTemplateParameters === null || themeTemplateParameters === void 0 ? void 0 : themeTemplateParameters.properties.get(ag_charts_community_1._Theme.DEFAULT_BACKGROUND_COLOUR);
