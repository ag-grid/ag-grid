...
class FiltersToolPanelListPanel extends core_1.Component {
    ...
    init(params) {
        ...
        this.addManagedListener(this.eventService, core_1.Events.EVENT_INTERNAL_TOOL_PANEL_VISIBLE_CHANGED, (event) => {
    ...
    recursivelyAddComps(tree, depth, expansionState) {
        return core_1._.flatten(tree.map(child => {
            if (child instanceof core_1.ProvidedColumnGroup) {
                return core_1._.flatten(this.recursivelyAddFilterGroupComps(child, depth, expansionState));
            }
            const column = child;
    ...
    recursivelyAddFilterGroupComps(columnGroup, depth, expansionState) {
        ...
        const childFilterComps = core_1._.flatten(this.recursivelyAddComps(columnGroup.getChildren(), newDepth, expansionState));
