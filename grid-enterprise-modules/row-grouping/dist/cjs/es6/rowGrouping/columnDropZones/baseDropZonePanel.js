...
const dropZoneColumnComp_1 = require("./dropZoneColumnComp");
class BaseDropZonePanel extends core_1.Component {
    constructor(horizontal, dropZonePurpose) {
        ...
        core_1._.setAriaRole(this.eColumnDropList, 'listbox');
    }
    isHorizontal() {
    ...
    removeColumns(columnsToRemove) {
        const newColumnList = this.getExistingColumns().filter(col => !core_1._.includes(columnsToRemove, col));
    ...
    getNonGhostColumns() {
        ...
        if (this.isPotentialDndColumns()) {
            return existingColumns.filter(column => !core_1._.includes(this.potentialDndColumns, column));
        }
    ...
    addAriaLabelsToComponents() {
        this.childColumnComponents.forEach((comp, idx) => {
            ...
            core_1._.setAriaSetSize(eGui, this.childColumnComponents.length);
        });
    }
    createColumnComponent(column, ghost) {
        const columnComponent = new dropZoneColumnComp_1.DropZoneColumnComp(column, this.dropTarget, ghost, this.dropZonePurpose, this.horizontal);
