...
let RangeService = class RangeService extends core_1.BeanStub {
    constructor() {
        super(...arguments);
        this.cellRanges = [];
    ...
    init() {
        ...
        this.addManagedListener(this.eventService, core_1.Events.EVENT_COLUMN_PIVOT_MODE_CHANGED, () => this.removeAllCellRanges());
        this.addManagedListener(this.eventService, core_1.Events.EVENT_COLUMN_ROW_GROUP_CHANGED, () => this.removeAllCellRanges());
        this.addManagedListener(this.eventService, core_1.Events.EVENT_COLUMN_PIVOT_CHANGED, () => this.removeAllCellRanges());
    ...
    onColumnsChanged() {
        ...
        this.cellRanges.forEach(cellRange => {
            ...
            const colsInRangeChanged = !core_1._.areEqual(beforeCols, cellRange.columns);
    ...
    refreshLastRangeStart() {
        const lastRange = core_1._.last(this.cellRanges);
        if (!lastRange) {
            return;
        }
    ...
    extendLatestRangeToCell(cellPosition) {
        ...
        const cellRange = core_1._.last(this.cellRanges);
    ...
    refreshRangeStart(cellRange) {
        ...
        const shouldMoveRightCol = startColumn === core_1._.last(columns) && startColumn === right;
    ...
    extendLatestRangeInDirection(event) {
        ...
        const lastCol = core_1._.last(lastRange.columns);
    ...
    clearCellRangeCellValues(params) {
        ...
        if (dispatchWrapperEvents) {
            const startEvent = {
                type: core_1.Events.EVENT_RANGE_DELETE_START,
        ...
        if (dispatchWrapperEvents) {
            const endEvent = {
                type: core_1.Events.EVENT_RANGE_DELETE_END,
                source: wrapperEventSource
            };
    ...
    isCellInSpecificRange(cell, range) {
        const columnInRange = range.columns !== null && core_1._.includes(range.columns, cell.column);
    ...
    isLastCellOfRange(cellRange, cell) {
        ...
        const rangeLastIndexColumn = core_1._.last(cellRange.columns);
    ...
    isBottomRightCell(cellRange, cell) {
        ...
        const { startRow, endRow } = cellRange;
        const lastRow = this.rowPositionUtils.before(startRow, endRow) ? endRow : startRow;
        const isRightColumn = allColumns.indexOf(cell.column) === core_1._.last(allPositions);
        const isLastRow = cell.rowIndex === lastRow.rowIndex && core_1._.makeNull(cell.rowPinned) === core_1._.makeNull(lastRow.rowPinned);
    ...
    updateValuesOnMove(eventTarget) {
        const cellCtrl = core_1._.getCtrlForEventTarget(this.gridOptionsService, eventTarget, core_1.CellCtrl.DOM_DATA_KEY_CELL_CTRL);
