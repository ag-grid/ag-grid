...
export function EventListener(eventName?: string): Function {
    return eventListenerFunc.bind(this, eventName);
}

function eventListenerFunc(eventName: string, target: Object, methodName: string, descriptor: TypedPropertyDescriptor<any>) {
    ...
    // it's an attribute on the class
    var props = getOrCreateProps(target);
    if (!props.eventListenerMethods) {
        props.eventListenerMethods = [];
    }
    props.eventListenerMethods.push({
