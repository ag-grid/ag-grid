...
<div>
    ...
    <p>As you can see in the example below, as soon as there is at least on active row group, the grid will provide an
        additional column for displaying the groups in a tree structure, with expand/collapse navigation.</p>
    ...
    <ul>
        ...
        <li>The country and sport columns used for grouping, are also shown as normal columns. You can hide them
        by adding <code>hide:true</code> to their <i>colDef</i>. This is illustrated in the Multi Auto Column example</li>
    </ul>
    </p>
    ...
    <p>
        The grid also lets you automatically create one group column for each column that you are grouping with
        <code>coldef.rowGroup = true</code>. This is achieved by setting <code>gridOptions.groupMultiAutoColumn = true</code>
        The following example illustrates this.
    </p>

    Note that:
    <ul>
        <li>There are a group column displayed for each columns that we are grouping by. One for every column with
            <code>rowGroup=true</code>, country and year</li>
        ...
        <li>The country and sport columns used for grouping, are hidden so that we don't show redundant information.
            They have <code>hide:true</code> added to their <i>colDef</i>. This is the opposite of the Auto Column Group example</li>
    </ul>
    </p>
    ...
    <p>
        You can specify you own configuration used by the auto group columns by providing a <code>gridOptions.groupColumnDef</code>,
        this can be used to override any property as defined in the <a href="../javascript-grid-column-definitions/">Columns</a>
        documentation page
    </p>

    <p>
        The auto generated columns generated by the grid use the out of the box group cell renderer, this means that
    ...
    <p>
        The following example illustrates how you can configure the auto group columns
    </p>

    Note that:
    <ul>
        <li>For the purpose of simplification this example uses one Auto Row Group Column, if you were to use Multi
            Auto Group Column the configuration would be applied to all the generated columns</li>
        <li>The header name of the group column is changed by setting in <i>groupColumnDef</i> <code>headerName:'CUSTOM!'</code></li>
        <li>The count for each group is removed. The group cell renderer used in the group column is configured by passing
            <i>groupColumnDef.cellRendererParams</i><code>suppressCount: true</code>
        </li>
        <li>Each group has a select box. The group cell renderer used in the group column is configured by passing
            <i>groupColumnDef.cellRendererParams</i><code>checkbox:true</code>
        </li>
        <li>
            The group column has a custom comparator that changes the way sorting works, this is achieved by providing
            in <i>groupColumnDef</i> <code>comparator: function (left, right){...}</code>
   ...
   <note>We advise you to use if you can auto group columns, they should cover most cases for row grouping</note>
...
coldefs:[
    ...
    <note>As you can see, and as we mentioned above, this examples are achieving the same that you can achieve with
    ...
    <p>
        As you can see from the previous examples, group columns don't produce values on the leaf nodes, the cells
    ...
    <p>
        ...
        node and then the filtering will be available based on this values.
    </p>
    ...
    <ul>
        ...
        are applying</li>
        <li>To see the leaf node values, open any country and any year group column, and note how their leaf nodes have
    ...
    <p>
        ...
        the order in which you provide the columns you are grouping by <code>rowGroup=true</code>. For instance as in
        all of our previous examples, if you were to provide your grouping columns like this:
<pre>gridOptions.columnDefs = [
    {headerName: "Country", field: "country", rowGroup:true},
    {headerName: "Sport", field: "sport", rowGroup:true},
];</pre>

        Any column that shows all the groups, will group by Country first, and then Sport since this is the order in
    which they arr defined.
    </p>

    <p>If for any reason, you would prefer to explicitly configure in which order you want to show the row groups in columns
    that show all the groups, then you can use <code>rowGroupIndex:index</code>. The index then will be used to work out
    the order of the groups. For instance, the following will revert the order of the previous example to first show
    'Sport' and then 'Country' even though 'Country' is defined first
<pre>gridOptions.columnDefs = [
    {headerName: "Country", field: "country", rowGroupIndex:1},
    {headerName: "Sport", field: "sport", rowGroupIndex:0},
];</pre>

    <note>You should stick with only one style of telling which columns you are grouping by, either be mindful of the
    order of your column declarations and use <code>rowGroup=true</code>. Or take full control and be in charge of
    specifying the order of the groups with <code>rowGroupIndex:index</code>. We recommend you to use
    <code>rowGroup=true</code> if possible</note>

    The following examples illustrates this, note how even though 'Sport' is defined after in the <i>colDefs</i> is
    the first group in the group column.
    ...
    <h4>Example Hide Open Parents with Explicit Group Columns</h4>
    ...
    <h4>Example Hide Open Parents with Auto Group Columns</h4>
    ...
    <h4>Full Width Groups Rendering</h4>
