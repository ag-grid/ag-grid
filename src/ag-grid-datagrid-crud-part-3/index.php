...
ng serve
            ...
            <p>First, let's install the ag-Grid dependencies - as we're going to be using a few of the Enterprise features
            that ag-Grid offers we'll install both the <code>ag-grid</code> and <code>ag-grid-enterprise</code> dependencies.</p>
            ...
            <p>The <code>ag-grid-angular</code> is what allows us to talk to the grid and provides the rich Angular functionality
            we want.</p>

            <p>We also need to let the Angular CLI know about the styles we want to use. In our demo we're going to use the
                <a href="https://www.ag-grid.com/javascript-grid-themes/fresh-theme.php">Fresh Theme</a>, but there are
                others available - please see the <a href="https://www.ag-grid.com/javascript-grid-styling/">Themes Documentation</a>
             for more details.</p>
...
"styles": [
    ...
    "../node_modules/ag-grid/dist/styles/theme-fresh.css"
            ...
            <p><code>styles.css</code> is a style file generated by Angular CLI - you can either keep it or remove it. We
...
...rest of module
            ...
            <p>We now have the ag-Grid dependencies all setup - our next step is to actually use ag-Grid to display some data.</p>
...
update src/app/app.module.ts
            ...
            will be responsible for supplying data to the Grid. Later, it will also be responsible for updates & deletions too.</p>
            ...
            <p>Finally, we'll hook into the <code>gridReady</code> event from the Grid. We do this for two reasons: firstly,
            to access the <code>GridApi</code> and <code>ColumnApi</code> and secondly to auto resize the columns on initialisation.</p>
...
&lt;/ag-grid-angular&gt;
            ...
            the <code>gridReady</code> event. There are other ways of doing this, but this is clearer and more idiomatic
            from an Angular perspective.</p>

            <p>As we're now using the <code>AthleteService</code> in our Grid Component, we can remove it from <code>app.component.ts</code>.</p>
            ...
            <p>The reason for this is pretty simple - both <code>Country</code> and <code>Results</code> are complex data.
            They don't have a simple key-value relationship unlike the rest of the data.</p>

            <p>We can fix this easily by making use of a <a href="https://www.ag-grid.com/javascript-grid-value-getters/">
...
private createColumnDefs() {
    return [
        ...
        {field: 'results', valueGetter: (params) => params.data.results.length}
            ...
            <p>Here the <code>valueGetter</code> callback will be called for every row for <code>country</code> and <code>results</code>,
            ...
            <p>That might have seemed like a fair bit of work, but it's worth noting that we only had to import a single module
            and then reference the grid in a single component to get a grid up and running.</p>

            <p>With the addition of a few simple properties we can enable filtering, sorting and so on. We can also start working
