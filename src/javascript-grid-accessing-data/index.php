...
<div>
    ...
    <pre><span class="codeComment">// callback tells the grid to use the 'id' attribute for id's</span>
<span class="codeComment">// id's should always be strings</span>
gridOptions.getRowNodeId = function(data) {
    return data.id;
};

<span class="codeComment">// get the row node with ID 55</span>
var rowNode = api.getRowNode('55');

<span class="codeComment">// do something with the row, eg select it</span>
rowNode.setSelected(true);
    ...
    <pre><span class="codeComment">// iterate through every node in the grid</span>
api.forEachNode( function(rowNode, index) {
    console.log('node ' + rowNode.data.athlete + ' is in the grid');
});

<span class="codeComment">// iterate only nodes that pass the filter</span>
api.forEachNodeAfterFilter( function(rowNode, index) {
    console.log('node ' + rowNode.data.athlete + ' passes the filter');
});

<span class="codeComment">// iterate only nodes that pass the filter and ordered by the sort order</span>
...
<span class="codeComment">// iterate through every leaf node in the grid</span>
api.forEachLeafNode( function(rowNode, index) {
    ...
    <show-example example="exampleGetDisplayedRow"></show-example>
