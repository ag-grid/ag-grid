...
BROWSER-3: After that salary edit, the amendment log shows one entry, and that entry names the
           record and shows the old value changing to the new value. Both values must be correct: an
           entry whose "from" value is the same as its "to" value, or which reports the wrong
           previous salary, is a fail.
