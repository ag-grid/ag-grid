...
export class GroupSelectsChildrenStrategy extends BeanStub implements ISelectionStrategy {
    ...
    public getSelectedState() {
        return this.recursivelyNormaliseState(this.selectedState, 0);
    }

    public setSelectedState(state: IServerSideGroupSelectionState) {
        const recursivelyConvertState = (normalisedState: IServerSideGroupSelectionState, parentSelected: boolean): SelectionState => {
            ...
            const convertedChildren = normalisedState.toggledNodes?.map<[string, SelectionState]>(innerState => (
                [innerState.nodeId!, recursivelyConvertState(innerState, isThisNodeSelected)]
            ));
        ...
        try {
            this.selectedState = recursivelyConvertState(state, !!state.selectAllChildren);
        } catch (e) {
            console.error(e.message);
        }
    }

    private recursivelyNormaliseState(state: SelectionState, level: number, nodeId?: string) {
        ...
        if (state.toggledNodes.size) {
            ...
            state.toggledNodes.forEach((value, key) => {
                const newState = this.recursivelyNormaliseState(value, level + 1, key);
    ...
    private recursivelySelectNode([nextNode, ...nodes]: IRowNode[], selectedState: SelectionState, params: { newValue: boolean, source: SelectionEventSourceType, event?: Event, node: RowNode }) {
        ...
        const cleanRedundantState = (childState: SelectionState) => {
            if (childState.selectAllChildren === selectedState.selectAllChildren && childState.toggledNodes.size === 0) {
                selectedState.toggledNodes.delete(nextNode.id!);
            }
        }
