...
export class ServerSideRowModel extends BeanStub implements IServerSideRowModel {
    ...
    private verifyProps(): void {
        ...
        if (this.gridOptionsWrapper.isRowSelection() && this.gridOptionsWrapper.getRowKeyFunc()==null) {
            const message = `AG Grid: getRowKey callback must be provided for Server Side Row Model selection to work correctly.`;
