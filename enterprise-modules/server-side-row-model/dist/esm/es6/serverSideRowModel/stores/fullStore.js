...
export class FullStore extends RowNodeBlock {
    ...
    initialiseRowNodes(loadingRowsCount = 1, failedLoad = false) {
        ...
        for (let i = 0; i < loadingRowsCount; i++) {
            const loadingRowNode = this.blockUtils.createRowNode({ field: this.groupField, group: this.groupLevel, leafGroup: this.leafGroup,
    ...
    createDataNode(data, index) {
        const rowNode = this.blockUtils.createRowNode({ field: this.groupField, group: this.groupLevel, leafGroup: this.leafGroup,
            level: this.level, parent: this.parentRowNode, rowGroupColumn: this.rowGroupColumn });
        if (index != null) {
    ...
    createOrRecycleNodes(nodesToRecycle, rowData) {
        ...
        const lookupNodeToRecycle = (dataItem) => {
            if (!nodesToRecycle) {
                return undefined;
            }
