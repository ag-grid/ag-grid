...
const partialStoreBlock_1 = require("../blocks/partialStoreBlock");
var FindResult;
(function (FindResult) {
...
class PartialStore extends core_1.BeanStub {
    constructor(ssrmParams, storeParams, parentRowNode) {
        ...
        this.displayIndexEnd = 0; // not sure if setting this one to zero is necessary
        this.cacheTopPixel = 0;
        this.info = {};
    ...
    onBlockLoadFailed(block) {
        block.setData([], true);
        this.fireCacheUpdatedEvent();
    }
    onBlockLoaded(block, params) {
        ...
        if (!params.rowData) {
            ...
            core_1._.doOnce(() => console.warn(message, params), 'InfiniteStore.noData');
        }
    ...
    purgeBlocksIfNeeded(blockToExclude) {
        ...
        blocksForPurging.forEach((block, index) => {
            const purgeBecauseBlockEmpty = block.getState() === partialStoreBlock_1.PartialStoreBlock.STATE_WAITING_TO_LOAD && index >= emptyBlocksToKeep;
    ...
    findBlockAndExecute(matchBlockFunc, blockFoundFunc, blockNotFoundFunc) {
    ...
    getRowBounds(index) {
        ...
        const blockFoundFunc = (foundBlock) => {
            return foundBlock.getRowBounds(index);
        };
        const blockNotFoundFunc = (previousBlock) => {
            let nextRowTop;
            let nextRowIndex;
    ...
    getRowIndexAtPixel(pixel) {
        ...
        let result = this.findBlockAndExecute(matchBlockFunc, blockFoundFunc, blockNotFoundFunc);
    ...
    setDisplayIndexes(displayIndexSeq, nextRowTop) {
        ...
        this.getBlocksInOrder().forEach(currentBlock => {
            ...
            for (let i = 1; i <= blocksSkippedCount; i++) {
                ...
                if (core_1._.exists(this.blockHeights[blockToAddId])) {
    ...
    getRowUsingDisplayIndex(displayRowIndex, dontCreateBlock = false) {
    ...
    getTopLevelRowDisplayedIndex(topLevelIndex) {
        ...
        const blockNotFoundFunc = (previousBlock) => {
            ...
            let lastDisplayedNodeIndexInBlockBefore;
    ...
    getCachedBlockHeight(blockNumber) {
        return this.blockHeights[blockNumber];
    }
    createBlock(blockNumber, displayIndex, nextRowTop) {
        const block = this.createBean(new partialStoreBlock_1.PartialStoreBlock(blockNumber, this.parentRowNode, this.ssrmParams, this.storeParams, this));
        block.setDisplayIndexes(new core_1.NumberSequence(displayIndex), nextRowTop);
    ...
    applyTransaction(transaction) {
        return { status: core_1.ServerSideTransactionResultStatus.StoreWrongType };
    }
    getChildStore(keys) {
        const findNodeCallback = (key) => {
            let nextNode = null;
            this.getBlocksInOrder().forEach(block => {
                block.forEachNodeShallow(rowNode => {
                    if (rowNode.key === key) {
    ...
    forEachChildStoreShallow(callback) {
        this.getBlocksInOrder().forEach(block => {
            if (block.isGroupLevel()) {
                const innerCallback = (rowNode) => {
                    const nextCache = rowNode.childStore;
                    if (nextCache) {
                        callback(nextCache);
                    }
                };
                block.forEachNodeShallow(innerCallback, new core_1.NumberSequence());
            }
        });
    }
}
...
exports.PartialStore = PartialStore;
