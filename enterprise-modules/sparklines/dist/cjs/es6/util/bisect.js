 ...
 * @param hi
 */
function bisectLeft(list, x, comparator, lo = 0, hi = list.length) {
    while (lo < hi) {
        const mid = (lo + hi) >>> 1;
...
function bisectRight(list, x, comparator, lo = 0, hi = list.length) {
    while (lo < hi) {
        const mid = (lo + hi) >>> 1;
        if (comparator(list[mid], x) > 0) { // list[mid] > x
            hi = mid;
 ...
 * @param hi
 */
function complexBisectLeft(list, x, map, lo = 0, hi = list.length) {
...
function complexBisectRight(list, x, map, lo = 0, hi = list.length) {
    const comparator = ascendingComparator(map);
    while (lo < hi) {
        const mid = (lo + hi) >>> 1;
        if (comparator(list[mid], x) < 0) {
