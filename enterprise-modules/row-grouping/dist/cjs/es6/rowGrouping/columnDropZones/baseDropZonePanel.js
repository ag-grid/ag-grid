...
class BaseDropZonePanel extends core_1.Component {
    ...
    getNewHorizontalInsertIndex(draggingEvent) {
        ...
        const goingLeft = draggingEvent.hDirection === core_1.HorizontalDirection.Left;
    ...
    getNewVerticalInsertIndex(draggingEvent) {
        ...
        this.childColumnComponents.forEach(childColumn => {
            const rect = childColumn.getGui().getBoundingClientRect();
            const verticalFit = mouseEvent.clientY >= (draggingEvent.vDirection === core_1.VerticalDirection.Down ? rect.top : rect.bottom);
            if (verticalFit) {
                newIndex++;
            }
        });
    ...
    createColumnComponent(column, ghost) {
        const columnComponent = new dropZoneColumnComp_1.DropZoneColumnComp(column, this.dropTarget, ghost, this.valueColumn, this.horizontal);
    ...
    addArrow(eParent) {
        ...
        if (this.horizontal) {
            ...
            const enableRtl = this.beans.gridOptionsWrapper.isEnableRtl();
            const icon = core_1._.createIconNoSpan(enableRtl ? 'smallLeft' : 'smallRight', this.beans.gridOptionsWrapper);
