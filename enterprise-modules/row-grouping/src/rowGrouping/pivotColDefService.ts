...
export class PivotColDefService extends BeanStub implements IPivotColDefService {
    ...
    private buildMeasureCols(
        ...
        return measureColumns.map((measureCol) => {
            const columnName = this.beans.columnModel.getDisplayNameForColumn(measureCol, 'header');
            return {
    ...
    private addExpandablePivotGroups(
        ...
        const recursivelyAddSubTotals = (
            ...
            if (group.children) {
                ...
                this.beans.columnModel.getValueColumns().forEach(valueColumn => {
                    const columnName: string | null = this.beans.columnModel.getDisplayNameForColumn(valueColumn, 'header');
    ...
    private addPivotTotalsToGroups(pivotColumnGroupDefs: (ColDef | ColGroupDef)[], pivotColumnDefs: ColDef[]) {
        ...
        const valueCols = this.beans.columnModel.getValueColumns();
    ...
    private generateColumnGroupId(pivotKeys: string[]): string {
        const pivotCols = this.beans.columnModel.getPivotColumns().map((col) => col.getColId());
    ...
    public createColDefsFromFields(fields: string[]): (ColDef | ColGroupDef)[] {
        ...
        const uniqueValuesToGroups = (id: string, key: string, uniqueValues: UniqueValue, depth: number): ColDef | ColGroupDef => {
            ...
            if (children.length === 0) {
                const potentialAggCol = this.beans.columnModel.getPrimaryColumn(key);
                if (potentialAggCol) {
                    const headerName = this.beans.columnModel.getDisplayNameForColumn(potentialAggCol, 'header') ?? key;
