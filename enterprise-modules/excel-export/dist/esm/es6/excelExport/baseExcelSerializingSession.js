...
export class BaseExcelSerializingSession extends BaseGridSerializingSession {
    ...
    onNewHeaderGroupingRow() {
        ...
        return {
            onColumn: (header, index, span, collapsibleRanges) => {
                const styleIds = this.config.styleLinker(RowType.HEADER_GROUPING, 1, `grouping-${header}`, undefined, undefined);
    ...
    onNewHeaderColumn(rowIndex, currentCells) {
        return (column, index) => {
            ...
            const styleIds = this.config.styleLinker(RowType.HEADER, rowIndex, nameForCol, column, undefined);
    ...
    onNewBodyColumn(rowIndex, currentCells) {
        ...
        return (column, index, node) => {
            ...
            const styleIds = this.config.styleLinker(RowType.BODY, rowIndex, valueForCell, column, node);
