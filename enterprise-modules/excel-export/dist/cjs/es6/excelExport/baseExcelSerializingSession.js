...
class BaseExcelSerializingSession extends csv_export_1.BaseGridSerializingSession {
    ...
    onNewHeaderGroupingRow() {
        ...
        return {
            onColumn: (columnGroup, header, index, span, collapsibleRanges) => {
                const styleIds = this.config.styleLinker({ rowType: csv_export_1.RowType.HEADER_GROUPING, rowIndex: 1, value: `grouping-${header}`, columnGroup });
    ...
    onNewHeaderColumn(rowIndex, currentCells) {
        return (column, index) => {
            ...
            const styleIds = this.config.styleLinker({ rowType: csv_export_1.RowType.HEADER, rowIndex, value: nameForCol, column });
