...
class BaseExcelSerializingSession extends csv_export_1.BaseGridSerializingSession {
    constructor(config) {
        super(config);
        this.mixedStyles = {};
    ...
    addCustomContent(customContent) {
        customContent.forEach(row => {
            ...
            this.rows.push({
                height: excelUtils_1.getHeightFromProperty(rowLen, this.config.rowHeight),
    ...
    onNewHeaderGroupingRow() {
        ...
        return {
            onColumn: (header, index, span, collapsibleRanges) => {
                const styleIds = this.config.styleLinker(csv_export_1.RowType.HEADER_GROUPING, 1, `grouping-${header}`, undefined, undefined);
    ...
    onNewHeaderColumn(rowIndex, currentCells) {
        return (column, index) => {
            ...
            const styleIds = this.config.styleLinker(csv_export_1.RowType.HEADER, rowIndex, nameForCol, column, undefined);
    ...
    onNewBodyColumn(rowIndex, currentCells) {
        ...
        return (column, index, node) => {
            ...
            if (!this.config.gridOptionsWrapper.isGroupHideOpenParents() && node.level) {
                core_1._.last(this.rows).outlineLevel = node.level;
            }
            const valueForCell = this.extractRowCellValue(column, index, rowIndex, core_1.Constants.EXPORT_TYPE_EXCEL, node);
            const styleIds = this.config.styleLinker(csv_export_1.RowType.BODY, rowIndex, valueForCell, column, node);
