...
const replaceHeaderFooterTokens = (value) => {
    ...
    core_1._.iterateObject(map, (key, val) => {
...
const buildHeaderFooter = (headerFooterConfig) => {
    ...
    rules.forEach(rule => {
        ...
        core_1._.iterateObject(headerFooter, (key, value) => {
...
const worksheetFactory = {
    getTemplate(params) {
        ...
        const createWorksheetChildren = core_1._.compose(addColumns(columns), addSheetData(rows, currentSheet + 1), addMergeCells(mergedCells), addPageMargins(margins), addPageSetup(pageSetup), addHeaderFooter(headerFooterConfig), addDrawingRel(currentSheet));
