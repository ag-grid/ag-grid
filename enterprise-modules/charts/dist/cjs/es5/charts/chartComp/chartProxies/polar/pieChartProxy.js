...
var PieChartProxy = /** @class */ (function (_super) {
    ...
    PieChartProxy.prototype.getSeries = function (params) {
        ...
        var series = this.getFields(params).map(function (f) {
            ...
            var options = __assign(__assign({}, seriesDefaults), { type: _this.standaloneChartType, angleKey: f.colId, angleName: f.displayName, calloutLabelKey: params.category.id, calloutLabelName: params.category.name });
            if (_this.chartType === 'doughnut') {
                ...
                return __assign(__assign({}, options), { outerRadiusOffset: outerRadiusOffset,
                    innerRadiusOffset: innerRadiusOffset, title: __assign(__assign({}, seriesDefaults.title), { text: seriesDefaults.title.text || f.displayName, showInLegend: numFields > 1 }), calloutLine: __assign(__assign({}, seriesDefaults.calloutLine), { colors: _this.chartTheme.palette.strokes }) });
            }
            return options;
        });
    ...
    PieChartProxy.prototype.extractCrossFilterSeries = function (series) {
        ...
        var primaryOptions = function (seriesOptions) {
            return __assign(__assign({}, seriesOptions), { calloutLabel: { enabled: false }, highlightStyle: { item: { fill: undefined } }, radiusKey: seriesOptions.angleKey, angleKey: seriesOptions.angleKey + '-total', radiusMin: 0, radiusMax: 1, listeners: __assign(__assign({}, seriesOverrides.listeners), { nodeClick: _this.crossFilterCallback }), tooltip: __assign(__assign({}, seriesOverrides.tooltip), { renderer: _this.getCrossFilterTooltipRenderer("" + seriesOptions.angleName) }) });
        };
        var filteredOutOptions = function (seriesOptions, angleKey) {
            var _a, _b, _c;
            return __assign(__assign({}, object_1.deepMerge({}, primaryOpts)), { radiusKey: angleKey + '-filtered-out', calloutLabel: seriesOverrides.calloutLabel, calloutLine: seriesOverrides.calloutLine && __assign(__assign({}, seriesOverrides.calloutLine), { colors: (_a = seriesOverrides.calloutLine.colors, (_a !== null && _a !== void 0 ? _a : palette.strokes)) }), fills: color_1.changeOpacity((_b = seriesOptions.fills, (_b !== null && _b !== void 0 ? _b : palette.fills)), 0.3), strokes: color_1.changeOpacity((_c = seriesOptions.strokes, (_c !== null && _c !== void 0 ? _c : palette.strokes)), 0.3), showInLegend: false });
        };
