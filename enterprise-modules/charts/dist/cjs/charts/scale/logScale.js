...
var LogScale = /** @class */ (function (_super) {
    ...
    function LogScale() {
        ...
        _this._domain = [1, 10];
        _this.baseLog = continuousScale_1.identity; // takes a log with base `base` of `x`
        _this.basePow = continuousScale_1.identity; // raises `base` to the power of `x`
        _this._base = 10;
        return _this;
    }
    ...
    LogScale.prototype.ticks = function (count) {
        ...
        if (!(base % 1) && p1 - p0 < count) {
            ...
            z = ticks_1.default(p0, p1, Math.min(p1 - p0, count)).map(this.basePow);
        }
