...
class SetFilter extends core_1.ProvidedFilter {
    ...
    addEventListenersForDataChanges() {
        this.addManagedListener(this.eventService, core_1.Events.EVENT_ROW_DATA_UPDATED, () => this.syncAfterDataChange());
    ...
    syncAfterDataChange(refreshValues = true) {
        if (!this.valueModel) {
            throw new Error('Value model has not been created.');
        }
        let promise = core_1.AgPromise.resolve();
        if (refreshValues) {
    ...
    onNewRowsLoaded() {
        ...
        this.syncAfterDataChange(valuesType === setValueModel_1.SetFilterModelValuesType.TAKEN_FROM_GRID_VALUES);
    }
