...
import { Autowired, Events, ProvidedFilter, RefSelector, VirtualList, AgPromise, KeyCode, PositionableFeature, _, } from '@ag-grid-community/core';
...
export class SetFilter extends ProvidedFilter {
    ...
    setParams(params) {
        ...
        this.valueModel = new SetValueModel(params, loading => this.showOrHideLoadingScreen(loading), this.valueFormatterService, key => this.translateForSetFilter(key), v => this.caseFormat(v));
    ...
    createSetListItem(value) {
        ...
        if (value === SetFilter.SELECT_ALL_VALUE) {
            listItem = this.createBean(new SetFilterListItem(() => this.getSelectAllLabel(), this.setFilterParams, key => this.translateForSetFilter(key), this.isSelectAllSelected()));
            listItem.addEventListener(SetFilterListItem.EVENT_SELECTION_CHANGED, (e) => this.onSelectAll(e.isSelected));
            return listItem;
        }
        listItem = this.createBean(new SetFilterListItem(value, this.setFilterParams, key => this.translateForSetFilter(key), this.valueModel.isValueSelected(value)));
        listItem.addEventListener(SetFilterListItem.EVENT_SELECTION_CHANGED, (e) => this.onItemSelected(value, e.isSelected));
        return listItem;
    }
    initMiniFilter() {
        if (!this.setFilterParams) {
