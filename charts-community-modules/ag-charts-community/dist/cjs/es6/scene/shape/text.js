...
function SceneFontChangeDetection(opts) {
    const { redraw = node_1.RedrawType.MAJOR, changeCb } = opts !== null && opts !== void 0 ? opts : {};
...
__decorate([
    node_1.SceneChangeDetection({ redraw: node_1.RedrawType.MAJOR, changeCb: (o) => o._setLines() })
...
function createTextMeasurer(font) {
    const cache = new Map();
    const getTextSize = (text) => hdpiCanvas_1.HdpiCanvas.getTextSize(text, font);
...
function getPreciseBBox(lines, x, y, textProps) {
    ...
    for (let i = 0; i < lines.length; i++) {
        const metrics = hdpiCanvas_1.HdpiCanvas.measureText(lines[i], font, textBaseline, textAlign);
...
function getApproximateBBox(lines, x, y, textProps) {
    ...
    const { lineHeight, textBaseline = Text.defaultStyles.textBaseline, textAlign = Text.defaultStyles.textAlign, } = textProps;
    if (lines.length > 0) {
        const lineSize = hdpiCanvas_1.HdpiCanvas.getTextSize(lines[0], font);
        width = lineSize.width;
        firstLineHeight = lineSize.height;
    }
    for (let i = 1; i < lines.length; i++) {
        const lineSize = hdpiCanvas_1.HdpiCanvas.getTextSize(lines[i], font);
