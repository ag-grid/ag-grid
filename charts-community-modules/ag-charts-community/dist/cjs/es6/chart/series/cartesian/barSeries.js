...
class BarSeries extends cartesianSeries_1.CartesianSeries {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            this.xData = data.map((datum) => {
                if (keysFound && !(xKey in datum)) {
                    keysFound = false;
                    logger_1.Logger.warn(`the key '${xKey}' was not found in the data: `, datum);
                }
            ...
            this.yData = data.map((datum) => yKeys.map((stack) => {
                return stack.map((yKey) => {
                    if (keysFound && !(yKey in datum)) {
                        keysFound = false;
                        logger_1.Logger.warn(`the key '${yKey}' was not found in the data: `, datum);
                    }
            ...
            let yMinMax;
            if (!isLogAxis) {
                yMinMax = this.yData.map((group) => group.map((stack) => array_1.findMinMax(stack)));
            }
            else {
                yMinMax = this.yData.map((group) => group.map((stack) => {
                    var _a;
                    const stackExtent = (_a = array_1.extent(stack)) !== null && _a !== void 0 ? _a : [];
                    return {
                        min: stackExtent[0],
                        max: stackExtent[1],
                    };
    ...
    getDomain(direction) {
        ...
        if (direction === chartAxisDirection_1.ChartAxisDirection.X) {
            ...
            const xDomain = array_1.extent(this.xData) || [NaN, NaN];
            if (flipXY) {
