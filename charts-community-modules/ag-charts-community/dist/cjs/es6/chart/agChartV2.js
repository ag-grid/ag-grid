...
function debug(message, ...optionalParams) {
    if ([true, 'opts'].includes(AgChartInternal.DEBUG())) {
        logger_1.Logger.debug(message, ...optionalParams);
    }
}
...
function applyModules(chart, options) {
    ...
    const rootModules = module_support_1.REGISTERED_MODULES.filter((m) => m.type === 'root');
...
function createSeries(options) {
    ...
    for (const seriesOptions of options || []) {
        ...
        switch (seriesOptions.type) {
            case 'area':
                series.push(applySeriesValues(new areaSeries_1.AreaSeries(), seriesOptions, { path, index }));
                break;
            case 'bar':
                series.push(applySeriesValues(new barSeries_1.BarSeries(), seriesOptions, { path, index }));
                break;
            case 'column':
                series.push(applySeriesValues(new barSeries_1.BarSeries(), seriesOptions, { path, index }));
                break;
            case 'histogram':
                series.push(applySeriesValues(new histogramSeries_1.HistogramSeries(), seriesOptions, { path, index }));
                break;
            case 'line':
                series.push(applySeriesValues(new lineSeries_1.LineSeries(), seriesOptions, { path, index }));
                break;
            case 'scatter':
                series.push(applySeriesValues(new scatterSeries_1.ScatterSeries(), seriesOptions, { path, index }));
                break;
            case 'pie':
                series.push(applySeriesValues(new pieSeries_1.PieSeries(), seriesOptions, { path, index }));
                break;
            case 'treemap':
                series.push(applySeriesValues(new treemapSeries_1.TreemapSeries(), seriesOptions, { path, index }));
                break;
            default:
