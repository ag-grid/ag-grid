import { GridApi, _ } from 'ag-grid-community';
import { MutableRefObject, useRef } from 'react';
import { AgGridReact } from './agGridReact';

const buildStub = <T>(stub: T) => {
    _.removeAllReferences(stub, 'GridApi', (key) =>`AG Grid: api.${key} was called on a destroyed grid (api.destroyed == true) resulting in a no-op.`);
    return stub;
};
