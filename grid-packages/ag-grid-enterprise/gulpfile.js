...
const copyGridCoreStyles = (done) => {
    if (!fs.existsSync('./node_modules/ag-grid-community/dist/styles/ag-grid.css')) {
        done("./node_modules/ag-grid-community/dist/styles doesn't exist - exiting")
    }

    return merge([
            gulp.src('./node_modules/ag-grid-community/dist/styles/*.css').pipe(gulp.dest('./dist/styles')),
            gulp.src('./node_modules/ag-grid-community/dist/styles/*.scss').pipe(gulp.dest('./dist/styles'))
        ]
    );
};
