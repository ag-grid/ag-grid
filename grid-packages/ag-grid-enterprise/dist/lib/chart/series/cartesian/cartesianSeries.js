...
var CartesianSeries = /** @class */ (function (_super) {
    ...
    CartesianSeries.prototype.validateXYData = function (xKey, yKey, data, xAxis, yAxis, xData, yData, yDepth) {
        ...
        if (isContinuousX && !hasNumber(xData)) {
            function_1.doOnce(function () { return console.warn("AG Charts - The number axis has no numeric data supplied for xKey: [" + xKey + "]."); }, 'series has no numeric data on number axis - ' + xKey);
            validationResult = false;
        }
        if (isContinuousY && !hasNumber(yData, 0, yDepth - 1)) {
            function_1.doOnce(function () { return console.warn("AG Charts - The number axis has no numeric data supplied for yKey: [" + yKey + "]."); }, 'series has no numeric data on number axis - ' + yKey);
            validationResult = false;
        }
