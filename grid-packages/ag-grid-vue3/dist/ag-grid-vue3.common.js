...
class AgGridColumn_AgGridColumn {
    ...
    static mapChildColumnDefs(slots) {
        return slots.default().map((column) => {
            return AgGridColumn_AgGridColumn.toColDef(column);
        });
    }
    static toColDef(column) {
        const colDef = AgGridColumn_AgGridColumn.createColDefFromGridColumn(column);
        if (column.children && column.children.length > 0) {
            colDef.children = AgGridColumn_AgGridColumn.getChildColDefs(column.children);
        }
        return colDef;
    }
    static getChildColDefs(columnChildren) {
        return columnChildren.map((column) => {
            return AgGridColumn_AgGridColumn.createColDefFromGridColumn(column);
        });
    }
    static createColDefFromGridColumn(column) {
        ...
        external_agGrid_["ColDefUtil"].BOOLEAN_PROPERTIES.forEach((property) => {
...
const AgGridVue = Object(external_commonjs_vue_commonjs2_vue_root_Vue_["defineComponent"])({
    ...
    mounted() {
        ...
        const frameworkComponentWrapper = new VueFrameworkComponentWrapper_VueFrameworkComponentWrapper(this, Object(external_commonjs_vue_commonjs2_vue_root_Vue_["getCurrentInstance"])() ? Object(external_commonjs_vue_commonjs2_vue_root_Vue_["getCurrentInstance"])().provides : null);
        ...
        const gridOptions = markRaw(external_agGrid_["ComponentUtil"].copyAttributesToGridOptions(toRaw(this.gridOptions), this));
