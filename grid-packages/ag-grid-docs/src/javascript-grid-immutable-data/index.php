<?php
$pageTitle = "Client-Side Data - Immutable Data";
...
<p>
    ...
    are what you should do if you want to make changes to the dataset rather than replace it. However
    this is not in line with how applications based on immutable stores desire to work.
</p>

<p>
    In applications using
    immutable stores (e.g. React and Redux) it could be desirable to treat changes to the bound
...
<p>
    For the Immutable Data Mode to work, you must be providing IDs for the row nodes as explained
    in <a href="../javascript-grid-row-node/#application-assigned-ids">Application Assigned IDs</a>.
</p>
...
<ul class="content">
    ...
    <li>
        Note that if a grid sort is applied, the grid sorting order gets preference to the order
        of the data in the provided list.
    </li>
    <li>
        <b>Remove Selected</b>: Removes the selected items. Try selecting multiple rows (<code>Ctrl</code> + click
        for multiple, or <code>Shift</code> + click for range) and remove multiple rows at the same time. Notice
...
<?= grid_example('Simple Immutable Store', 'simple-immutable-store', 'generated', ['enterprise' => true, 'exampleHeight' => 540, 'modules' => ['clientside', 'rowgrouping']]) ?>
...
<ul class="content">
    ...
    <li>
        Note that all grid state (row and range selections, filters, sorting etc.) remain after
        updates are applied.
    </li>
</ul>

<?= grid_example('Complex Immutable Store', 'complex-immutable-store', 'generated', ['enterprise' => true, 'exampleHeight' => 590, 'modules' => ['clientside', 'rowgrouping']]) ?>
...
<ul>
    ...
    <li>
        ...
        when it comes to Immutable Data Mode. If you want a grid that manages high frequency data changes, it is
