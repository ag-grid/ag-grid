...
<ul class="content">
    <li>
        <code>grid-size</code> is the main control for affecting how tightly data and UI elements are packed together. All padding and spacing 
        in the grid is defined as a multiple of grid-size, so increasing it will make most components larger by increasing their internal white space 
        while leaving the size of text and icons unchanged.
    </li>
    <li>
        <code>borders</code> controls whether borders are drawn around the grid. There are more <code>border-*</code> parameters to provide fine-grained 
        control over which borders are drawn and their colour.
    </li>
    <li>
        <code>row-height</code> height in pixels of a grid row.
    </li>
    <li>
    ...
    <ul>
        <li>
            <code>alpine-active-color</code> (Alpine only) sets the colour of checked checkboxes, range selections, row selections, selected tab underlines, 
            and input focus outlines
        </li>
        <li>
            <code>balham-active-color</code> (Balham only) sets the colour of checked checkboxes, range selections, row selections, and input focus outlines
        </li>
        <li>
...
<p>
    ...
    <a href="https://github.com/ag-grid/ag-grid-customise-theme/tree/master/src/vanilla">general webpack example</a> appropriate Vanilla JS and React projects, 
    and an <a href="https://github.com/ag-grid/ag-grid-customise-theme/tree/master/src/angular">angular example</a> using Angular CLI.
</p>
...
<p>
    Note how this example includes the structural styles (<code>ag-gris.scss</code>) before the theme mixin. Doing this means that both 
    structural and theme styles will be included in the compiled CSS file. Alternatively, you could leave out the first <code>@import</code>
    and then embed the structural stylesheet separately in your HTML page.
</p>
...
<ol class="content">
    <li>
        Firstly, the grid will attempt to measure the size of an element. This works when styles have loaded, but will not work if the grid 
        initialises before the theme loads. Our <a href="https://github.com/ag-grid/ag-grid-customise-theme/blob/master/src/vanilla/grid.js">theme customisation examples</a> 
        demonstrate how to wait for CSS to load before initialising the grid (see the cssHasLoaded function).
    </li>
    <li>
    ...
    <li>
        ...
        not using a provided theme with heights unchanged) then you should inform the grid about your custom element heights using 
        <a href="/javascript-grid-properties/">grid properties</a>. The minimal set of properties you need to set to ensure correct 
        functioning are: <code>rowHeight</code>, <code>headerHeight</code> and <code>minColWidth</code>.
    </li>
</ol>

<h2 id="setting-parameters-css-variables">Setting colour parameters using CSS variables</h2>

<p>
    CSS variables (officially known to as "CSS Custom Properties") are supported by most modern browsers but will not work in IE11. Any parameter
    whose name ends with <code>-color</code> is available as a CSS variable with the prefix <code>--ag-</code>. For example the <code>foreground-color</code> 
...
<p>
    This will cause the text in grid cells to be set at runtime to the value of the <code>--myDataColorVar</code>. If both a custom variable (<code>--myDataColorVar</code>) 
    and a built in variable (<code>--ag-foreground-color</code>) are set at runtime, the built in variable will take priority. If you don't want this to happen then you 
    can disable built in variables for your theme by setting the parameter <code>suppress-css-var-overrides: false</code>.
</p>

<p>
    We have created an example that demonstrates both methods of CSS variable use in our 
    <a href="https://github.com/ag-grid/ag-grid-customise-theme/tree/master/src/vanilla-css-variables">theme examples GitHub repo</a>.
</p>
...
<p>
    ...
    parameters alone. For example, there is no parameter to set the <code>font-style: italic</code> on header cells. If you want your 
    column headers to be italic, use regular CSS:
</p>
...
<p>
    ...
    have multiple class names, some more general than others. For example, the <a href="/javascript-grid-tool-panel-columns/#column-tool-panel-example">row grouping panel</a> 
    is a component onto which you can drag columns to group them. The internal name for this is the "column drop" component, and there are two 
    kinds - a horizontal one at the top of the header and a vertical one in the columns tool panel. You can use the class name <code>ag-column-drop</code> 
    to target either kind, or <code>ag-column-drop-vertical</code> / <code>ag-column-drop-horizontal</code> to target one only.
</p>

<h3>Understanding CSS rule maintenance and breaking changes</h3>

<p>
    With each release of the grid we add features and improve existing ones, and as a result the DOM structure changes with every release - even 
    minor and patch releases. Of course we test and update the CSS rules in our themes to make sure they still work, and this includes ensuring that 
...
<ul>
    <li>
        Visual styles including margins, paddings, sizes, colours, fonts, borders etc are all fine to change
        in a theme.
    </li>
    <li>
        Setting a component to <code>display: flex</code> and changing flex child layout properties like
        ...
        text or icons within a container; but if you're trying to change the layout of the grid on a larger scale e.g. 
        turning a vertical scrolling list into a horizontal one, you are likely to break Grid features.
    </li>
    <li>
        The style properties <code>position</code>, <code>overflow</code> and <code>pointer-events</code>
...
<p>
    The default values in this list demonstrate the kind of value that is expected (a colour, pixel value, percentage value etc) 
    but bear in mind that if you are using a provided theme then the theme will have changed most of the default values - you can 
    find the default values for your theme by inspecting its source code in the grid distribution - look for a file called 
    <code>_ag-theme-{theme-name}-default-params.scss</code>.
</p>

<p>
    Note that some values are defined relative to other values using the <code>ag-derived</code> helper function, so 
    <code>data-color: ag-derived(foreground-color)</code> means that if you don't set the <code>data-color</code> property it 
    will default to the value of <code>foreground-color</code>. See the <a href="#ag-derived">ag-derived docs</a> for more information.
</p>
...
<p>
    ...
    value of the parameter that it derives from, as in the case of <code>disabled-foreground-color</code> above. This requires the value to be 
...
<p>
    The following theme functions and mixins are available if you are include a theme mixin file like <code>ag-theme-alpine-mixin.scss</code>, or can be used in isolation 
    by importing <code>styles/mixins/_ag-theme-params.scss</code> from the grid distribution.
</p>
...
<p>
    The <code>ag-color-property</code> mixin takes 3 arguments: a CSS property name, a parameter name, and an optional boolean indicating whether the rule 
    should be marked as <code>!important</code>. Like <code>ag-param</code> it can be used to reference the value of a parameter, but it additionally adds 
...
<p>
    As the user of a theme you do not need to write code including the <code>ag-derived</code> function, but it's still useful to know about 
    it so that you can understand your theme's default parameters. Our provided themes make extensive use of this function to implement 
    <a href="#parameter-cascading">parameter cascading</a>.
</p>

<p>
    If you're writing a theme that is intended to be extended and used by other developers, you may use this function to implement your own 
    parameter cascades.
</p>

<p>
    Parent themes define a Sass map of supported parameter names and their default values, which can optionally be overridden by child themes 
    by passing params to the parent theme mixin:
</p>
...
<p>
    The <code>ag-derived</code> function supports modifiers - named arguments that can be used to modify the value of the parameter 
    being derived from. In this case, if the <code>cat-color</code> parameter is not overridden, it will default to a semitransparent 
    version of animal-color:
</p>
...
<p>
    Instead of passing a value to a modifier, it is also possible to pass a parameter name. In this case, the <code>cat-color</code> parameter 
    defaults to a semitransparent version of animal-color with the exact amount of opacity depending on the value of <code>cat-opacity</code>:
</p>
...
<ul class="content">
    ...
    <ul>
        ...
        <li>
            <code>$lighten: percentage</code> - amount should be a percentage from 0% to 100%, equivalent to Sass <code>lighten($value, $amount)</code>
        </li>
        <li>
