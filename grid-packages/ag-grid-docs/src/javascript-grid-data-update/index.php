...
?>
    ...
    <p class="lead">
        There are many ways in which data can change in your application - as a result many ways in which you
    ...
    <p>
        Updating data in the grid via the grid's API does not cover all the ways in which dat can change
        inside the grid. Data can also change in the grid in the following ways:
    </p>

    <ol class="content">
        <li>
            Editing data inside the grid using the grid's UI, eg by the user double clicking on a cell and
            editing the cells value. When this happens the grid is in control and there is no need to
            explicitly tell the grid data has changed. See
            <a href="../javascript-grid-cell-editing">in-line editing</a> on how to editing via the grid's UI.
        </li>
        <li>
            Updating the grid's data is updated from elsewhere in your application. This can happen if you pass data
    ...
    <p>
        ...
        e.g. loading a new report with a completely different data set to the previous. This makes sure nothing
    ...
    <ul>
        <li>
            ...
            <p>
                Updates a single rows or cells value. This is done by getting a reference
