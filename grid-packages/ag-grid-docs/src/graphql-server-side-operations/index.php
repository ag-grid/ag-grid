...
<p>
    In this guide we will develop an Olympic Medals application that demonstrates how to integrate a GraphQL endpoint
...
<p>
    ...
    with GraphQL is assumed, however this <a href="https://graphql.org/learn/">Introduction to GraphQL</a>
...
<?= createSnippet('git clone https://github.com/ag-grid/ag-grid-server-side-graphql-example.git', 'sh') ?>

<p>
    Navigate into the project directory:
</p>

<?= createSnippet('cd ag-grid-server-side-graphql-example', 'sh') ?>

<p>
    Install project dependencies and build project using:
</p>

<?= createSnippet('yarn install', 'sh') ?>
...
<p>
    Create a database with the name <code>'sample_data'</code>. Then run the following script to create the table
    <code>olympic_winners</code> and populate it with data via the MySQL command line:
</p>

<?= createSnippet('mysql -u root -p -D sample_data < ./data/olympic_winners.sql', 'sh') ?>
...
<p>
    A single <code>rows</code> query is also defined along with its supported input types and enums:
</p>
...
<p>
    The GraphQL schema is created using the <code>makeExecutableSchema</code> helper function from the
