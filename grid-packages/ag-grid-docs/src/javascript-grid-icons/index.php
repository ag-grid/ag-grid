...
<p>
    ...
    The following code snippet overrides the Alpine theme pin icon used in the drag hint when reordering columns:
</p>
...
<p>
    If you are using a <a href="/javascript-grid-styling/">custom theme</a> in your project, you can use theme
    parameters to change the icon font. We
    <a href="https://github.com/ag-grid/ag-grid-customise-theme/tree/master/src/vanilla">provide an example</a>
...
<p>
    If you are swapping one theme's icon set for another, you do not need to define an icon map because all theme fonts
    use the same map. This example shows the use of Alpine with the Material font:
</p>
...
<p>
    A working project with Sass / Webpack set up to customise an icon set is available in the
    <a href="https://github.com/ag-grid/ag-grid-customise-theme">ag grid customising theme repository</a>.
</p>
...
<p>
    As of version 23, checkboxes and radio buttons are native browser inputs styled using CSS. This means that you can
    change the appearance of the checkbox with Sass, but not using the JavaScript <code>gridOptions</code> technique.
    Using Sass, you can either change the icon font (set the <code>checkbox-*</code> and <code>radio-button-*</code>
...
<?= grid_example('Icons', 'icons', 'generated', ['enterprise' => true, 'exampleHeight' => 660, 'extras' => ['fontawesome'], 'modules' => ['clientside', 'rowgrouping'], 'reactFunctional' => true]) ?>

<h2>SVG Icons</h2>

<p>
    When you create your own theme as described in <a href="../javascript-grid-themes-customising/">Customising Themes</a>,
    you are also able to replace the WebFont with SVG Icons. To do that you will need to override th
     <code>ag-icon</code> SASS rules and also the rules for each icon. You can see the example <code>styles.scss</code>
     file in our custom theme with SVG icons example here:
    <a href="https://github.com/ag-grid/ag-grid-customise-theme/tree/master/src/vanilla-svg-icons">SVG Icons Example</a>.
</p>

<note>
    The grid uses the CSS <code>color</code> property to change the colour of icons. This works for webfont-based icons,
