...
$pageKeywords = "ag-Grid ViewPort";
$pageGroup = "row_models";
include '../documentation-main/documentation_header.php';
?>

<h1 class="heading-enterprise">Viewport Row Model</h1>
...
<p>
    To enable the Viewport Row Model, set the grid property <code>rowModelType='viewport'</code>.
</p>

<p>
    The term 'viewport' is a common term in GUIs used to describe the visible area when scrolls are used
...
<p>
    ...
    the viewport information to the Viewport Row Model.
</p>
...
<p>
    ...
    (typically a server), with the source sending updates to the grid when displayed data is changed.
</p>

<p>
    The diagram below shows how the viewport maps to a connection to your dataset. The dataset connection
    knows what the viewport is displaying and sends data accordingly. When the user scrolls, the viewport
...
<img src="./viewport.png" class="img-fluid" alt="Viewport Connected to Dataset"/>
...
<p>
    To use the viewportRowModel you provide the grid with a <code>viewportDatasource</code>, which
...
<ol class="content">
    <li>
        You provide a new <code>viewportDatasource</code> to the grid. The grid will call <code>datasource.init()</code>
    ...
    <li>
        The datasource responds with the size of the data (e.g. 1,000 rows) and calls <code>params.setRowCount(1000)</code>.
...
<ul class="content">
    <li>
        ...
        inside the <code>rowNode</code> replacing the old data. This will result in the grid
        removing the rendered row from the DOM and replacing with a new rendered row. If you have any custom <code>cellRenderers</code>, they
        will be destroyed and new ones created, so there is no way to achieve animation.
    </li>
    <li>
        <code>rowNode.setDataValue(colKey, newValue)</code>: Call this to set one value into the node. The new value will
        be stored inside the old data, leaving the rest of the data untouched. This will result in the grid refreshing
        only the cell that was updated. If the cell has a component with a refresh method, the refresh method will be
        called, allowing animation. If no refresh method is provided, the grid will remove the cell (and destroy
...
<p>
    If you want to change the length of data (e.g. you apply a filter, or the result set otherwise grows or shrinks) then you
...
<p>
    Only server-side sorting is supported, so if you want sorting you have to do it yourself on the server.
...
<p>
    As with sorting, filtering also must be done on the server. To implement, listen for the <code>filterChanged</code>
...
<p>
    And you guessed it, if you are doing grouping, you will need to implement this yourself on the server.
    If you group, you will need to provide your own <code>groupCellRenderer</code> that gives functionality to your own
    custom grouping. You will also need to manage how the grouping impacts the overall grid's set size yourself (i.e.
...
<p>
    For simplicity, we stated earlier that the viewport was the rows the grid is currently displaying. This is <i>almost</i> true
    except there are two properties, <code>viewportRowModelPageSize</code> and <code>viewportRowModelBufferSize</code>, to make the
...
<p>
    ...
    grid hits 'the next five rows'. The default page size is <code>5</code>. To change this, set the grid property
...
<p>
    ...
    The default buffer size is <code>5</code>. To change this, set the grid property <code>viewportRowModelBufferSize</code>.
</p>
...
<p>
    Two built-in cell renderers are used: <code>animateShowChange</code> (bid, mid and ask columns) and <code>animateSlide</code>
    ...
    as how the animation happens will depend on your application, the type of data and frequency
    of change.
</p>

<p>
    The example uses a <code>mockServer</code>. This is because all of the examples in this documentation work
    without any dependencies on any server. In your application, instead of using a mock server, you should
    connect to your real server. However the code in the <code>mockServer</code> example can be used to observe what
...
<ul class="content">
    ...
    <li>
        ...
        be scrolling, so there is no need to set a minimum page size. Setting page size to <code>1</code> means the grid
