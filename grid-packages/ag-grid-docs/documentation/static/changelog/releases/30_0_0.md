...
- AG-8505 - Improved title, subtitle, footnote and `treemap` label wrapping options and strategies by adding `wrapping`, `maxWidth` and `maxHeight` properties into the options structure.
- AG-8110 - Legend item synchronisation allows matching items to be toggled together, and can be configured via the `series[].legendItemKey` or `series[].legendItemName` options.
- AG-6809 - Label formatters added to scatter, pie, and treemap series.
- AG-7112 - Axis title formatter added.
- AG-5568 - Axis labels can now be toggled on or off via the `enabled` property.
- AG-8364 - Tooltip arrow can now be disabled via `showArrow` property.
- AG-8451 - Improved build tooling compatibility by replacing use of `eval()` with `new Function()` for templated decorator function generation.
...
Certain properties defined on the `defaultColDef`, including `valueFormatter` and `valueParser`, will now be overridden by cell data types (which are enabled by default). The `valueFormatter` will also not run on group column values when cell data types are enabled (as these have already been formatted). See [Cell Data Types](https://ag-grid.com/archive/30.0.0/javascript-data-grid/cell-data-types/) for more information.
...
Tool Panel
  *   `toolPanelVisibleChanged` updated, see [toolPanelVisibleChanged](https://ag-grid.com/archive/30.0.0/javascript-data-grid/tool-panel/#reference-accessories-toolPanelVisibleChanged). It now fires twice when switching between tool panels (once for the old panel and once for the new panel), but has new fields to indicate the change. The source property now refers to the event source rather than the panel name.

### Grid API
Callbacks which had the generic `TValue` type may now be slightly stricter with regards to null values. See [Cell \<TValue>](https://ag-grid.com/archive/30.0.0/javascript-data-grid/typescript-generics/#cell-value-tvalue).
