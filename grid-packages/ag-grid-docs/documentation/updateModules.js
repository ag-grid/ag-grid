...
const moduleMapping = [
  ...
  {
    title:
      "Column Tool Panel & Column Menu Panel<enterprise-icon></enterprise-icon>",
  ...
  {
    title:
      "Row Grouping, Pivoting & Tree Data<enterprise-icon></enterprise-icon>",
...
function findFilesWithSameNameInSubfolder(filePath, docFile, subFolderPath) {
  ...
  for (const match of matches) {
    ...
    fs.readFile(examplePath, "utf8", (err, data) => {
      ...
      if (
        data.includes("ModuleRegistry.registerModules") ||
