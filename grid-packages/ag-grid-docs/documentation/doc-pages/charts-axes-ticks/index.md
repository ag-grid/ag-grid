...
*Note: Sometimes, when the difference between `minSpacing` and `maxSpacing` is small, the actual spacing between the ticks will appear outside of the requested range. This is because ticks are generated with the interval of 1x, 2x, 5x, 10x (or minute, hour, 12 hours, etc. for time axis). Please set `maxSpacing` to be 2x-3x times larger than `minSpacing`, or use `tick.interval` or `tick.values` options instead.*
