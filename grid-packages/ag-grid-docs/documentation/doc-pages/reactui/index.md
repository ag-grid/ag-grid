...
1. The `refresh()` method is no longer used for Cell Comps when using ReactUI. Instead the props are updated in the Cell Renderer, which results in React doing a render on the Cell Renderer component.
