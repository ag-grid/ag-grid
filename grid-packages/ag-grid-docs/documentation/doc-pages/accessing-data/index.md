...
## Accessing RowNode by Key

The easiest way to get a Row Node is by its Key (sometimes called Row ID). The Key is either provided by you using the grid callback `getRowKey()`, or generated by the grid using an internal sequence.
