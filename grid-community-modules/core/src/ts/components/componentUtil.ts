...
export class ComponentUtil {
    ...
    public static processOnChange(changes: any, api: GridApi): void {
        ...
        const updates = Object.keys(changesToApply).map(key => {
            ...
            const prevValue = api.__setProperty(gridKey, coercedValue);
            return {gridKey, coercedValue, prevValue}
        });
        // Then firing property change events if required.
        updates.forEach((u) => api.__updateProperty(u.gridKey, u.coercedValue, u.prevValue, this.changeSetId));
