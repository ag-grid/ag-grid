...
var DefaultDateComponent = /** @class */ (function (_super) {
    ...
    DefaultDateComponent.prototype.setParams = function (params) {
        ...
        if (minValidDate && minValidYear) {
            function_1.doOnce(function () { return console.warn('AG Grid: DateFilter should not have both minValidDate and minValidYear parameters set at the same time! minValidYear will be ignored.'); }, 'DateFilter.minValidDateAndMinValidYearWarning');
        }
        if (maxValidDate && maxValidYear) {
            function_1.doOnce(function () { return console.warn('AG Grid: DateFilter should not have both maxValidDate and maxValidYear parameters set at the same time! maxValidYear will be ignored.'); }, 'DateFilter.maxValidDateAndMaxValidYearWarning');
        }
        if (minValidDate && maxValidDate) {
            var _b = __read([minValidDate, maxValidDate]
                .map(function (v) { return v instanceof Date ? v : date_1.parseDateTimeFromString(v); }), 2), parsedMinValidDate = _b[0], parsedMaxValidDate = _b[1];
            if (parsedMinValidDate && parsedMaxValidDate && parsedMinValidDate.getTime() > parsedMaxValidDate.getTime()) {
                function_1.doOnce(function () { return console.warn('AG Grid: DateFilter parameter minValidDate should always be lower than or equal to parameter maxValidDate.'); }, 'DateFilter.minValidDateAndMaxValidDateWarning');
            }
        }
        if (minValidDate) {
            if (minValidDate instanceof Date) {
                inputElement.min = date_1.dateToFormattedString(minValidDate);
            }
            else {
        ...
        if (maxValidDate) {
            if (maxValidDate instanceof Date) {
                inputElement.max = date_1.dateToFormattedString(maxValidDate);
            }
            else {
