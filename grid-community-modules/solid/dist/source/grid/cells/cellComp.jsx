...
const CellComp = (props) => {
    ...
    const bodyJsxFunc = () => (<>
                <For each={renderCompVersionList()}>{() => <>
                        {renderDetails() && <ShowRenderDetails showDetails={renderDetails()} ref={cellRenderer} cellInstanceId={cellInstanceId} showCellWrapper={showCellWrapper()} cellCtrl={cellCtrl} includeDndSource={includeDndSource()} includeRowDrag={includeRowDrag()} includeSelection={includeSelection()} setSelectionCheckboxId={setSelectionCheckboxId} showTools={showTools()} setECellValue={setECellValue}/>}
                    </>}</For>
                {editDetails() &&
