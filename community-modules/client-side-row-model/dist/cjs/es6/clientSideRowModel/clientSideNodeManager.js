...
class ClientSideNodeManager {
    constructor(rootNode, gridOptionsWrapper, eventService, columnModel, gridApi, columnApi, selectionService, beans) {
        this.nextId = 0;
    ...
    lookupRowNode(data) {
        ...
        if (getRowIdFunc) {
            ...
            const id = getRowIdFunc({ data, level: 0, api: this.gridApi, columnApi: this.columnApi });
    ...
    createNode(dataItem, parent, level) {
        const node = new core_1.RowNode(this.beans);
        node.group = false;
