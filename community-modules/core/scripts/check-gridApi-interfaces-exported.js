...
function checkGridOptionPropertyKeys() {
    ...
    if (missingPropertyKeys.size > 0) {
        console.error(
            'check-grid-api-exports - GridApi are using types that are not publicly exported. Missing the following types:',
            [...missingPropertyKeys].join(', ')
        );
        console.error(
            'If running locally and you have added the missing type be sure to run build in "grid-community-modules/core", "grid-packages/ag-grid-community", "grid-packages/ag-grid-enterprise".'
        );
        return 1;
    }
...
function getPublicTypes(fileContents) {
    ...
    publicTypes = [
        ...new Set(publicTypes.filter((i) => !toIgnore.includes(i)).filter((s) => s[0] === s[0].toUpperCase())),
    ].sort();
    return publicTypes;
}
