...
var rowComp_1 = require("./row/rowComp");
...
var RowRenderer = /** @class */ (function (_super) {
    ...
    RowRenderer.prototype.onPageLoaded = function (refreshEvent) {
        if (generic_1.missing(refreshEvent)) {
            refreshEvent = {
    ...
    RowRenderer.prototype.getAllCellsForColumn = function (column) {
        ...
        object_1.iterateObject(this.rowCompsByIndex, callback);
        object_1.iterateObject(this.floatingBottomRowComps, callback);
        object_1.iterateObject(this.floatingTopRowComps, callback);
        return eCells;
    };
    ...
    RowRenderer.prototype.getRenderedIndexesForRowNodes = function (rowNodes) {
        ...
        object_1.iterateObject(this.rowCompsByIndex, function (index, renderedRow) {
    ...
    RowRenderer.prototype.getCellToRestoreFocusToAfterRefresh = function (params) {
        ...
        if (generic_1.missing(focusedCell)) {
            return null;
        }
        ...
        var elementIsNotACellDev = generic_1.missing(domData);
    ...
    RowRenderer.prototype.forEachCellCompFiltered = function (rowNodes, columns, callback) {
        ...
        object_1.iterateObject(this.rowCompsByIndex, function (index, rowComp) {
            processRow(rowComp);
        });
    ...
    RowRenderer.prototype.binRowComps = function (recycleRows) {
        ...
        if (recycleRows) {
            ...
            object_1.iterateObject(this.rowCompsByIndex, function (index, rowComp) {
                var rowNode = rowComp.getRowNode();
                if (generic_1.exists(rowNode.id)) {
    ...
    RowRenderer.prototype.calculateIndexesToDraw = function (rowsToRecycle) {
        ...
        object_1.iterateObject(this.rowCompsByIndex, checkRowToDraw);
    ...
    RowRenderer.prototype.redraw = function (rowsToRecycle, animate, afterScroll) {
        ...
        indexesToDraw.forEach(function (rowIndex) {
            ...
            if (generic_1.exists(rowComp)) {
                rowComps.push(rowComp);
                array_1.pushAll(nextVmTurnFunctions, rowComp.getAndClearNextVMTurnFunctions());
            }
        });
        this.flushContainers(rowComps);
        function_1.executeNextVMTurn(nextVmTurnFunctions);
    ...
    RowRenderer.prototype.flushContainers = function (rowComps) {
        object_1.iterateObject(this.rowContainers, function (key, rowContainerComp) {
            if (rowContainerComp) {
    ...
    RowRenderer.prototype.redrawFullWidthEmbeddedRows = function () {
        ...
        var rowsToRemove = [];
        object_1.iterateObject(this.rowCompsByIndex, function (id, rowComp) {
            if (rowComp.isFullWidth()) {
    ...
    RowRenderer.prototype.destroyRowComps = function (rowCompsMap, animate) {
        var delayedFuncs = [];
        object_1.iterateObject(rowCompsMap, function (nodeId, rowComp) {
            // if row was used, then it's null
            if (!rowComp) {
                return;
            }
            rowComp.destroy(animate);
            array_1.pushAll(delayedFuncs, rowComp.getAndClearDelayedDestroyFunctions());
        });
        function_1.executeInAWhile(delayedFuncs);
    };
    ...
    RowRenderer.prototype.workOutFirstAndLastRowsToRender = function () {
        ...
        if (!this.paginationProxy.isRowsToRender()) {
            ...
            var firstPixel = Math.max(bodyTopPixel + paginationOffset - bufferPixels, pageFirstPixel) + maxDivHeightScaler;
            var lastPixel = Math.min(bodyBottomPixel + paginationOffset + bufferPixels, pageLastPixel) + maxDivHeightScaler;
    ...
    RowRenderer.prototype.tryToFocusFullWidthRow = function (position, backwards) {
        ...
        var rowComp = this.getRowCompByPosition(position);
    ...
    RowRenderer.prototype.getRowCompByPosition = function (rowPosition) {
        var rowComponent;
    ...
    RowRenderer.prototype.getComponentForCell = function (cellPosition) {
        var rowComponent = this.getRowCompByPosition(cellPosition);
        if (!rowComponent) {
            return null;
        }
    ...
    RowRenderer.prototype.onTabKeyDown = function (previousRenderedCell, keyboardEvent) {
        ...
        if (success) {
            ...
            if (firstRow) {
                ...
                var column = array_1.last(this.columnController.getAllDisplayedColumns());
            ...
            // previous cell.
            if (previousRenderedCell instanceof cellComp_1.CellComp) {
    ...
    RowRenderer.prototype.tabToNextCell = function (backwards) {
        ...
        var renderedComp = this.getComponentForCell(focusedCell);
        // if cell is not rendered, means user has scrolled away from the cell
        // or that the focusedCell is a Full Width Row
        if (generic_1.missing(renderedComp)) {
    ...
    RowRenderer.prototype.moveToNextEditingCell = function (previousRenderedCell, backwards) {
        ...
        var foundCell = generic_1.exists(nextRenderedCell);
    ...
    RowRenderer.prototype.moveToNextCellNotEditing = function (previousRenderedCell, backwards) {
        var displayedColumns = this.columnController.getAllDisplayedColumns();
        var gridCell;
        if (previousRenderedCell instanceof rowComp_1.RowComp) {
            gridCell = __assign(__assign({}, previousRenderedCell.getRowPosition()), { column: backwards ? displayedColumns[0] : array_1.last(displayedColumns) });
        }
        else {
    ...
    RowRenderer.prototype.findNextCellToFocusOn = function (gridCell, backwards, startEditing) {
        ...
        while (true) {
            ...
            if (!nextCellComp) {
                var rowComp = this.getRowCompByPosition(nextCell);
                if (!rowComp || !rowComp.isFullWidth()) {
                    continue;
                }
                else {
