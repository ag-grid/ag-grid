...
let instanceIdSeq = 0;
let PopupService = PopupService_1 = class PopupService extends beanStub_1.BeanStub {
    constructor() {
        super(...arguments);
        this.popupList = [];
    }
    postConstruct() {
        this.ctrlsService.whenReady(p => {
            ...
            this.addManagedListener(this.gridCtrl, events_1.Events.EVENT_KEYBOARD_FOCUS, () => {
                this.popupList.forEach(popup => popup.element.classList.add(focusService_1.FocusService.AG_KEYBOARD_FOCUS));
            });
            this.addManagedListener(this.gridCtrl, events_1.Events.EVENT_MOUSE_FOCUS, () => {
                this.popupList.forEach(popup => popup.element.classList.remove(focusService_1.FocusService.AG_KEYBOARD_FOCUS));
            });
        });
    }
    getPopupParent() {
    ...
    keepXYWithinBounds(ePopup, position, direction) {
        ...
        const getSize = isVertical ? dom_1.getAbsoluteHeight : dom_1.getAbsoluteWidth;
    ...
    addPopup(params) {
        ...
        const hidePopup = (popupParams = {}) => {
            ...
            this.eventService.removeEventListener(events_1.Events.EVENT_DRAG_STARTED, hidePopupOnMouseEvent);
            if (closedCallback) {
        ...
        window.setTimeout(() => {
            ...
            if (modal) {
                ...
                this.eventService.addEventListener(events_1.Events.EVENT_DRAG_STARTED, hidePopupOnMouseEvent);
