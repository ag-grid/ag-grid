...
class AgColorPanel extends component_1.Component {
    constructor(config) {
    ...
    update() {
        const color = utils_1.Color.fromHSB(this.H * 360, this.S, this.B, this.A);
        const spectrumColor = utils_1.Color.fromHSB(this.H * 360, 1, 1);
        const rgbaColor = color.toRgbaString();
        // the recent color list needs to know color has actually changed
        const colorPicker = this.picker;
        const existingColor = utils_1.Color.fromString(colorPicker.getValue());
    ...
    setValue(val) {
        const color = utils_1.Color.fromString(val);
