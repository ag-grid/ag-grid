...
let ColumnModel = class ColumnModel extends beanStub_1.BeanStub {
    ...
    setColumnsVisible(keys, visible = false, source = "api") {
        this.columnAnimationService.start();
        this.actionOnGridColumns(keys, (column) => {
    ...
    orderColumnStateList(columnStateList) {
        ...
        const gridColumnIdMap = map_1.convertToMap(this.gridColumns.map((col, index) => [col.getColId(), index]));
    ...
    applyColumnState(params, source = "api") {
        ...
        if (params.state) {
            params.state.forEach((state) => {
                const groupAutoColumnId = constants_1.Constants.GROUP_AUTO_COLUMN_ID;
    ...
    orderGridColsLikeLastPrimary() {
        ...
        const lastPrimaryOrderMapped = map_1.convertToMap(this.lastPrimaryOrder.map((col, index) => [col, index]));
    ...
    // called from api
    sizeColumnsToFit(gridWidth, source = "sizeColumnsToFit", silent) {
