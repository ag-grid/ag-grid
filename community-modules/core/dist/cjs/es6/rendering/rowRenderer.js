...
const rowCtrl_1 = require("./row/rowCtrl");
...
const stickyRowFeature_1 = require("./features/stickyRowFeature");
const browser_1 = require("../utils/browser");
let RowRenderer = class RowRenderer extends beanStub_1.BeanStub {
    constructor() {
        super(...arguments);
    ...
    initialise() {
        ...
        this.addManagedListener(this.gridOptionsWrapper, gridOptionsWrapper_1.GridOptionsWrapper.PROP_ROW_CLASS, this.redrawRows.bind(this));
        if (this.gridOptionsWrapper.isGroupRowsSticky()) {
            if (this.rowModel.getType() != constants_1.Constants.ROW_MODEL_TYPE_CLIENT_SIDE) {
                function_1.doOnce(() => console.warn('AG Grid: The feature Sticky Row Groups only works with the Client Side Row Model'), 'rowRenderer.stickyWorksWithCsrmOnly');
            }
            else if (this.gridOptionsWrapper.isTreeData()) {
                function_1.doOnce(() => console.warn('AG Grid: The feature Sticky Row Groups does not work with Tree Data.'), 'rowRenderer.stickyDoesNotWorkWithTreeData');
            }
            else {
    ...
    updateAllRowCtrls() {
        const liveList = object_1.getAllValuesInObject(this.rowCtrlsByRowIndex);
        if (this.gridOptionsWrapper.isEnsureDomOrder()) {
            liveList.sort((a, b) => a.getRowNode().rowIndex - b.getRowNode.rowIndex);
        }
        const zombieList = object_1.getAllValuesInObject(this.zombieRowCtrls);
    ...
    registerCellEventListeners() {
        this.addManagedListener(this.eventService, events_1.Events.EVENT_CELL_FOCUSED, (event) => {
            this.getAllCellCtrls().forEach(cellCtrl => cellCtrl.onCellFocused(event));
            this.getFullWidthRowCtrls().forEach(rowCtrl => {
                rowCtrl.onFullWidthRowFocused(event);
            });
        });
        this.addManagedListener(this.eventService, events_1.Events.EVENT_FLASH_CELLS, event => {
            this.getAllCellCtrls().forEach(cellCtrl => cellCtrl.onFlashCells(event));
        });
        this.addManagedListener(this.eventService, events_1.Events.EVENT_COLUMN_HOVER_CHANGED, () => {
            this.getAllCellCtrls().forEach(cellCtrl => cellCtrl.onColumnHover());
        });
        this.addManagedListener(this.eventService, events_1.Events.EVENT_DISPLAYED_COLUMNS_CHANGED, () => {
        ...
        this.addManagedListener(this.eventService, events_1.Events.EVENT_DISPLAYED_COLUMNS_WIDTH_CHANGED, () => {
            if (this.printLayout) {
        ...
        if (rangeSelectionEnabled) {
            this.addManagedListener(this.eventService, events_1.Events.EVENT_RANGE_SELECTION_CHANGED, () => {
                this.getAllCellCtrls().forEach(cellCtrl => cellCtrl.onRangeSelectionChanged());
            });
            this.addManagedListener(this.eventService, events_1.Events.EVENT_COLUMN_MOVED, () => {
                this.getAllCellCtrls().forEach(cellCtrl => cellCtrl.updateRangeBordersIfRangeCount());
            });
            this.addManagedListener(this.eventService, events_1.Events.EVENT_COLUMN_PINNED, () => {
                this.getAllCellCtrls().forEach(cellCtrl => cellCtrl.updateRangeBordersIfRangeCount());
            });
    ...
    refreshListenersToColumnsForCellComps() {
        ...
        cols.forEach(col => {
            ...
            col.addEventListener(column_1.Column.EVENT_COL_DEF_CHANGED, colDefChangedListener);
            this.destroyFuncsForColumnListeners.push(() => {
                ...
                col.removeEventListener(column_1.Column.EVENT_COL_DEF_CHANGED, colDefChangedListener);
            });
        });
    }
    onDomLayoutChanged() {
    ...
    refreshFloatingRows(rowComps, rowNodes) {
        ...
        rowNodes.forEach(rowNode => {
            const rowCtrl = new rowCtrl_1.RowCtrl(rowNode, this.beans, false, false, this.printLayout);
    ...
    getRenderedIndexesForRowNodes(rowNodes) {
        ...
        object_1.iterateObject(this.rowCtrlsByRowIndex, (index, renderedRow) => {
    ...
    getCellToRestoreFocusToAfterRefresh(params) {
        ...
        const cellDomData = this.gridOptionsWrapper.getDomData(activeElement, cellCtrl_1.CellCtrl.DOM_DATA_KEY_CELL_CTRL);
        const rowDomData = this.gridOptionsWrapper.getDomData(activeElement, rowCtrl_1.RowCtrl.DOM_DATA_KEY_ROW_CTRL);
    ...
    redrawAfterModelUpdate(params = {}) {
        ...
        const isFocusedCellGettingRecycled = () => {
            ...
            let res = false;
            object_1.iterateObject(rowsToRecycle, (key, rowComp) => {
    ...
    isRowInMap(rowNode, rowIdsMap) {
        ...
        if (floating === constants_1.Constants.PINNED_TOP) {
    ...
    calculateIndexesToDraw(rowsToRecycle) {
        ...
        let indexesToDraw = number_1.createArrayOfNumbers(this.firstRenderedRow, this.lastRenderedRow);
    ...
    dispatchDisplayedRowsChanged() {
        const event = { type: events_1.Events.EVENT_DISPLAYED_ROWS_CHANGED };
    ...
    getFullWidthRowCtrls(rowNodes) {
        ...
        return object_1.getAllValuesInObject(this.rowCtrlsByRowIndex).filter((rowCtrl) => {
    ...
    destroyRowCtrls(rowCtrlsMap, animate) {
        ...
        object_1.iterateObject(rowCtrlsMap, (nodeId, rowCtrl) => {
    ...
    workOutFirstAndLastRowsToRender() {
        ...
        const rowLayoutNormal = this.gridOptionsWrapper.getDomLayout() === constants_1.Constants.DOM_LAYOUT_NORMAL;
