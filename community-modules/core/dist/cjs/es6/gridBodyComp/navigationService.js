...
let NavigationService = class NavigationService extends beanStub_1.BeanStub {
    ...
    isTimeSinceLastPageEventToRecent() {
        const now = new Date().getTime();
        const diff = now - this.timeLastPageEventProcessed;
        return (diff < 100);
    }
    setTimeLastPageEventProcessed() {
    ...
    getIndexToFocus(indexToScrollTo, isDown) {
    ...
    onCtrlLeftOrRight(key, gridCell) {
        const leftKey = key === keyCode_1.KeyCode.LEFT;
        const allColumns = this.columnModel.getAllDisplayedColumns();
        const isRtl = this.gridOptionsWrapper.isEnableRtl();
        const columnToSelect = leftKey !== isRtl ? allColumns[0] : array_1.last(allColumns);
        this.navigateTo({
    ...
    tabToNextCellCommon(previous, backwards) {
    ...
    moveToNextEditingCell(previousCell, backwards) {
    ...
    moveToNextEditingRow(previousCell, backwards) {
        const previousPos = previousCell.getCellPosition();
        // find the next cell to start editing
        const nextCell = this.findNextCellToFocusOn(previousPos, backwards, true);
        if (nextCell == null) {
