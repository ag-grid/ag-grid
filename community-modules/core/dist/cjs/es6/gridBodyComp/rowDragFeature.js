...
class RowDragFeature extends beanStub_1.BeanStub {
    ...
    getRowNodes(draggingEvent) {
        ...
        const selectedNodes = [...this.selectionService.getSelectedNodes()].sort((a, b) => this.getRowIndexNumber(a) - this.getRowIndexNumber(b));
    ...
    moveRowAndClearHighlight(draggingEvent) {
        ...
        const isBelow = lastHighlightedRowNode && lastHighlightedRowNode.highlighted === rowNode_1.RowHighlightPosition.Below;
        const pixel = this.mouseEventService.getNormalisedPosition(draggingEvent).y;
        const rowNodes = draggingEvent.dragItem.rowNodes;
        ...
        if (this.isFromThisGrid(draggingEvent)) {
            ...
            this.clientSideRowModel.updateRowData({
                add: rowNodes
                    .map(node => node.data)
                    .filter(data => !this.clientSideRowModel.getRowNode(getRowIdFunc ? getRowIdFunc({ data, level: 0, api: this.gridApi, columnApi: this.columnApi }) : data.id)),
                addIndex
            });
        }
