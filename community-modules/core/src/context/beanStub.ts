...
export class BeanStub extends Destroyable implements IEventEmitter {
    ...
    /**
     * Enable beans to be provided for classes that extend BeanStub but are not actually a bean themselves such as RowCtrl.
     * These items are manually created for performance reasons.
     * Does not need to be called for beans that are created via the context.
     * @param beans
     */
    protected manualSetBeans(beans: Beans): void {
        (this.beans as any) = beans;
    }
    ...
    public addManagedEventListener(event: EventsType, listener: AgEventListener): (() => null) | undefined {
        return this.addManagedListener(this.beans.eventService, event, listener);
    }
    ...
    private setupGridOptionListener<K extends keyof GridOptions>(
        ...
        this.beans.gos.addEventListener(event, listener);
        const destroyFunc: () => null = () => {
            this.beans.gos.removeEventListener(event, listener);
            return null;
        };
        this.destroyFunctions.push(destroyFunc);
