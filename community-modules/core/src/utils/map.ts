export function _convertToMap<K, V>(arr: [K, V][]): Map<K, V> {
...
export function _mapById<V>(arr: V[], callback: (obj: V) => string): Map<string, V> {
...
export function _keys<T>(map: Map<T, any>): T[] {
    const arr: T[] = [];
