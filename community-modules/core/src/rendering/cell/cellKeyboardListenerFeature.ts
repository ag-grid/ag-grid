...
export class CellKeyboardListenerFeature extends BeanStub {
    ...
    #onNavigationKeyDown(event: KeyboardEvent, key: string): void {
    ...
    #onShiftRangeSelect(event: KeyboardEvent): void {
    ...
    #onBackspaceOrDeleteKeyDown(key: string, event: KeyboardEvent): void {
        const cellCtrl = this.#cellCtrl;
        const rowNode = this.#rowNode;
        const { gos, rangeService, eventService } = this.#beans;
    ...
    #onF2KeyDown(event: KeyboardEvent): void {
        if (!this.#cellCtrl.isEditing()) {
