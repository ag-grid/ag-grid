...
export class PaginationProxy extends BeanStub {
    ...
    #pageSizeAutoCalculated?: number; // When paginationAutoPageSize = true or when the pages panel is disabled
    #pageSizeFromPageSizeSelector?: number; // When user selects page size from page size selector.
    #pageSizeFromInitialState?: number; // When the initial grid state is loaded, and a page size rehydrated
    #pageSizeFromGridOptions?: number; // When user sets gridOptions.paginationPageSize.
    #defaultPageSize: 100; // When nothing else set, default page size is 100.

    #totalPages: number;
    #currentPage = 0;

    #topDisplayedRowIndex = 0;
    #bottomDisplayedRowIndex = 0;
    #pixelOffset = 0;
    #topRowBounds: RowBounds;
    #bottomRowBounds: RowBounds;

    #masterRowCount: number = 0;
    ...
    #onModelUpdated(modelUpdatedEvent?: WithoutGridCommon<ModelUpdatedEvent>): void {
        this.#calculatePages();
    ...
    #onPaginationGridOptionChanged(): void {
        this.#active = this.gos.get('pagination');
        this.#calculatePages();
        const paginationChangedEvent: WithoutGridCommon<PaginationChangedEvent> = {
            type: Events.EVENT_PAGINATION_CHANGED,
    ...
    #onPageSizeGridOptionChanged(): void {
