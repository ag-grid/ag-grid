...
export class ApiEventService extends BeanStub {
    ...
    public addEventListener(eventType: string, userListener: AgEventListener): void {
        ...
        const async = this.beans.gos.useAsyncEvents() && !ALWAYS_SYNC_GLOBAL_EVENTS.has(eventType);;
        const listeners = async ? this.asyncEventListeners : this.syncEventListeners;
        if (!listeners.has(eventType)) {
            listeners.set(eventType, new Set());
        }
        listeners.get(eventType)!.add(listener);
        this.beans.eventService.addEventListener(eventType, listener, async);
    }
    public removeEventListener(eventType: string, userListener: AgEventListener): void {
        ...
        this.beans.eventService.removeEventListener(eventType, listener, hasAsync);
    }
    ...
    public addGlobalListener(userListener: AgGlobalEventListener): void {
        ...
        if (async) {
            ...
            this.beans.eventService.addGlobalListener(syncListener, false);
            this.beans.eventService.addGlobalListener(asyncListener, true);
        } else {
            this.syncGlobalEventListeners.add(listener);
            this.beans.eventService.addGlobalListener(listener, false);
        }        
    }
    ...
    public removeGlobalListener(userListener: AgGlobalEventListener): void {
        ...
        if (hasAsync) {
            ...
            this.beans.eventService.removeGlobalListener(syncListener, false);
            this.beans.eventService.removeGlobalListener(asyncListener, true);
            this.globalEventListenerPairs.delete(userListener);
        } else {
            this.syncGlobalEventListeners.delete(listener);
            this.beans.eventService.removeGlobalListener(listener, false);
        }
    }

    private destroyEventListeners(map: Map<string, Set<AgEventListener>>, async: boolean): void {
        map.forEach((listeners, eventType) => {
            listeners.forEach(listener => this.beans.eventService.removeEventListener(eventType, listener, async));
            listeners.clear();
        });
        map.clear();
    }
    ...
    private destroyGlobalListeners(set: Set<AgGlobalEventListener>, async: boolean): void {
        set.forEach(listener => this.beans.eventService.removeGlobalListener(listener, async));
        set.clear();
    }
