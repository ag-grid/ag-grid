...
export class SortController extends BeanStub {
    ...
    public setSortForColumn(column: Column, sort: SortDirection, multiSort: boolean, source: ColumnEventType): void {
        ...
        if (isColumnsSortingCoupledToGroup) {
            if (column.getColDef().showRowGroup) {
                const rowGroupColumns = this.functionColumnsService.getSourceColumnsForGroupColumn(column);
    ...
    private updateSortIndex(lastColToChange: Column) {
        ...
        this.columnModel.getPrimaryAndPivotResultAndAutoColumns().forEach(col => col.setSortIndex(null));
    ...
    public isSortActive(): boolean {
        ...
        const allCols = this.columnModel.getPrimaryAndPivotResultAndAutoColumns();
    ...
    private clearSortBarTheseColumns(columnsToSkip: Column[], source: ColumnEventType): Column[] {
        ...
        this.columnModel.getPrimaryAndPivotResultAndAutoColumns().forEach((columnToClear: Column) => {
    ...
    private getIndexedSortMap(): Map<Column, number> {
        // pull out all the columns that have sorting set
        let allSortedCols = this.columnModel.getPrimaryAndPivotResultAndAutoColumns()
        ...
        const sortedRowGroupCols = this.functionColumnsService.getRowGroupColumns()
            .filter(col => !!col.getSort());
    ...
    public getDisplaySortForColumn(column: Column): SortDirection | 'mixed' | undefined {
        const linkedColumns = this.functionColumnsService.getSourceColumnsForGroupColumn(column);
