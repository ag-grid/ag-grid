...
var getAgGridProperties = () => {
  const props2 = {};
  const eventNameAsProps = import_core.ComponentUtil.PUBLIC_EVENTS.map((eventName) => kebabNameToAttrEventName(kebabProperty(eventName)));
  eventNameAsProps.forEach((eventName) => props2[eventName] = void 0);
  const computed2 = {
    props() {
      const options = {};
      import_core.ComponentUtil.ALL_PROPERTIES.forEach((propertyName) => {
        var _a;
        if (this[propertyName] === import_core.ComponentUtil.VUE_OMITTED_PROPERTY) {
          return;
        }
  ...
  const watch2 = {
    modelValue: {
      handler(currentValue, previousValue) {
        ...
        import_core.ComponentUtil.processOnChange({ rowData: currentValue }, this.api);
      },
      deep: true
    },
    props: {
      handler(currentValue, previousValue) {
        ...
        import_core.ComponentUtil.processOnChange(changes, this.api);
      },
      deep: true
    }
  };
...
var VueComponentFactory = class _VueComponentFactory {
  static getComponentDefinition(component, parent) {
    ...
    if (typeof component === "string") {
      ...
      componentDefinition = { extends: (0, import_vue.defineComponent)(__spreadValues({}, component)) };
    }
...
// community-modules/vue3/src/VueFrameworkComponentWrapper.ts
var VueFrameworkComponentWrapper = class _VueFrameworkComponentWrapper extends import_core2.BaseComponentWrapper {
...
var VueFrameworkOverrides = class extends import_core3.VanillaFrameworkOverrides {
  constructor(parent) {
    super("vue");
    this.parent = parent;
  }
...
var ALWAYS_SYNC_GLOBAL_EVENTS = /* @__PURE__ */ new Set([import_core4.Events.EVENT_GRID_PRE_DESTROYED]);
...
var AgGridVue = (0, import_vue2.defineComponent)({
  ...
  methods: {
    ...
    processChanges(propertyName, currentValue, previousValue) {
      if (this.gridCreated) {
        ...
        import_core4.ComponentUtil.processOnChange(options, this.api);
      }
    },
  ...
  mounted() {
    ...
    const gridOptions = markRaw(import_core4.ComponentUtil.combineAttributesAndGridOptions(toRaw(this.gridOptions), this));
    this.checkForBindingConflicts();
    const rowData = this.getRowDataBasedOnBindings();
    if (rowData !== import_core4.ComponentUtil.VUE_OMITTED_PROPERTY) {
    ...
    this.api = (0, import_core4.createGrid)(this.$el, gridOptions, gridParams);
