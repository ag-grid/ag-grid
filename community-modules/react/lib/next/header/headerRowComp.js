...
var HeaderRowComp = function (props) {
    ...
    var setCellCtrlsMaintainOrder = react_1.useCallback(function (prev, next) {
        ...
        var prevMap = core_1._.mapById(prev, function (c) { return c.getInstanceId(); });
        var nextMap = core_1._.mapById(next, function (c) { return c.getInstanceId(); });
    ...
    typeFilter && react_1.useEffect(function () {
        ...
        cellCtrls.forEach(function (cellCtrl) {
            ...
            if (existing) {
                ...
                var newComp = context.createBean(new core_1.HeaderFilterCellComp(cellCtrl));
                eGui.current.appendChild(newComp.getGui());
            }
        });
        var oldComps = core_1._.getAllValuesInObject(oldCompsMap);
        destroyJsFilterComps(oldComps);
    }, [cellCtrls]);
    // when row comp is destroyed, make sure no child floating filters left
    typeFilter && react_1.useEffect(function () {
        var cellCtrls = [];
        core_1._.getAllValuesInObject(jsFilterCells.current)
