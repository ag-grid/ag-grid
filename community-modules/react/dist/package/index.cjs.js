...
var CellEditorComponentProxy = class {
  constructor(cellEditorParams, refreshProps) {
    ...
    this.refreshProps = refreshProps;
    this.instanceCreated = new import_core11.AgPromise((resolve) => {
      this.resolveInstanceCreated = resolve;
    });
      ...
      const addComp = (comp) => {
        if (comp) {
          ...
          cellWrapperDestroyFuncs.current.push(() => {
            context.destroyBean(comp);
            (0, import_core12._removeFromParent)(eGui2);
          });
        }
        return comp;
      };
...
var RowContainerComp = (params) => {
  ...
  const { name } = params;
  const containerOptions = (0, import_react18.useMemo)(() => (0, import_core14._getRowContainerOptions)(name), [name]);
  ...
  const setRef2 = (0, import_react18.useCallback)(() => {
    ...
    if (areElementsReady()) {
      ...
      rowContainerCtrlRef.current = context.createBean(new import_core14.RowContainerCtrl(name));
...
var GridBodyComp = () => {
  ...
  if (!cssClassManager.current) {
    cssClassManager.current = new import_core15.CssClassManager(() => eRoot.current);
  }
  ...
  const setRef2 = (0, import_react19.useCallback)((e) => {
    ...
    addComp(eRoot.current, import_core15.FakeHScrollComp, " AG Fake Horizontal Scroll ");
    addComp(eRoot.current, import_core15.OverlayWrapperComponent, " AG Overlay Wrapper ");
    if (eBody.current) {
      addComp(eBody.current, import_core15.FakeVScrollComp, " AG Fake Vertical Scroll ");
    }
    const compProxy = {
      setRowAnimationCssOnBodyViewport: setRowAnimationClass,
      setColumnCount: (count) => {
        if (eRoot.current) {
          (0, import_core15._setAriaColCount)(eRoot.current, count);
        }
      },
      setRowCount: (count) => {
        if (eRoot.current) {
          (0, import_core15._setAriaRowCount)(eRoot.current, count);
        }
      },
      setTopHeight,
      setBottomHeight,
    ...
    const ctrl = context.createBean(new import_core15.GridBodyCtrl());
...
var TabGuardCompRef = (props, forwardRef4) => {
  ...
  const setupCtrl = (0, import_react20.useCallback)(() => {
    ...
    if (topTabGuardRef.current && bottomTabGuardRef.current) {
      ...
      tabGuardCtrlRef.current = context.createBean(
        new import_core16.TabGuardCtrl({
          comp: compProxy,
  ...
  const createTabGuard = (side) => {
    const className = side === "top" ? import_core16.TabGuardClassNames.TAB_GUARD_TOP : import_core16.TabGuardClassNames.TAB_GUARD_BOTTOM;
    return /* @__PURE__ */ import_react20.default.createElement(
      "div",
      {
        className: `${import_core16.TabGuardClassNames.TAB_GUARD} ${className}`,
        role: "presentation",
...
var GridComp = ({ context }) => {
  ...
  const setRef2 = (0, import_react21.useCallback)((e) => {
    ...
    gridCtrlRef.current = context.createBean(new import_core17.GridCtrl());
...
var RenderStatusService = class extends import_core18.BeanStub {
  wireBeans(beans) {
...
var AgGridReactUi = (props) => {
  ...
  const setRef2 = (0, import_react22.useCallback)((e) => {
    ...
    const mergedGridOps = (0, import_core19._combineAttributesAndGridOptions)(props.gridOptions, props);
  ...
  (0, import_react22.useEffect)(() => {
    ...
    processWhenReady(() => {
      if (apiRef.current) {
        (0, import_core19._processOnChange)(changes, apiRef.current);
      }
    });
  }, [props]);
