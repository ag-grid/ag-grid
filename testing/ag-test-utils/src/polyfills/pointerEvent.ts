...
function ensureDragEvent(): void {
    // Capability, not existence: happy-dom aliases DragEvent to plain Event, so the constructor is there
    // but drops `dataTransfer` (and every MouseEvent coordinate) — a drop handler then sees no files.
    const Ctor = (globalThis as any).DragEvent;
    let carriesDataTransfer: boolean;
    try {
        carriesDataTransfer =
            typeof Ctor === 'function' &&
            new Ctor('dragstart', { dataTransfer: new DataTransfer() }).dataTransfer != null;
    } catch {
        carriesDataTransfer = false;
    }
    if (!carriesDataTransfer) {
