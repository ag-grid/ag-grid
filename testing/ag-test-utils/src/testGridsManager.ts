...
import { ignoreConsoleLicenseKeyError } from './ignoreKnownNoise';
...
export class TestGridsManager {
    public static defaultGridOptions: GridOptions = {
        ...
        // happy-dom does not have a layout engine, so, elements don't have size (width/height are 0 and not computed)
    ...
    public reset(): void {
        ...
        _resetMissingModuleReports(); // Fresh page: clear the missing-module batch and its dedup
    }
    ...
    /** The grid's own outermost element, which is what DOM validation and harness queries walk down from. */
    public static getHTMLElement(api: GridApi | null | undefined): HTMLElement | null {
        return api ? ((getGridElement(api) as HTMLElement | undefined) ?? null) : null;
    }
}
