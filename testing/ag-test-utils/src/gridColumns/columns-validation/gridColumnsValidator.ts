...
export class GridColumnsValidator {
    ...
    public validate(gridColumns: GridColumns): void {
        ...
        this.validateSortIndices(allGridCols.length ? allGridCols : allDisplayedCols);
    ...
    private validatePivotResultColumns(gridColumns: GridColumns): void {
        ...
        // Column ids are user-controlled, so the `pivot_` prefix alone cannot decide this: it is only a
        // fallback for a leaked result column the api no longer lists, never a verdict on a declared one.
        const declaredColIds = collectDeclaredColIds(api.getGridOption?.('columnDefs'));

        for (let i = 0, len = gridColumns.allDisplayedCols.length; i < len; ++i) {
            const col = gridColumns.allDisplayedCols[i];
            const colId = col.getColId();
            const isPivotResult =
                (pivotResultSet?.has(col) ?? false) || (colId.startsWith('pivot_') && !declaredColIds.has(colId));
            if (!isPivotResult) {
                continue;
            }
            if (!pivotMode) {
    ...
    /** Over ALL grid columns: a hidden sorted column owns a real sortIndex, so excluding it from the
     *  sequence makes a valid grid look non-sequential. */
...
/** Every colId the user declared, groups included: `colId` when given, else `field`. */
function collectDeclaredColIds(columnDefs: unknown): Set<string> {
    const ids = new Set<string>();
    const walk = (defs: unknown): void => {
        if (!Array.isArray(defs)) {
            return;
        }
        for (const def of defs as Record<string, unknown>[]) {
