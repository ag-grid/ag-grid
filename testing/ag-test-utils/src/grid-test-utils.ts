...
export function isAgHtmlElementVisible(element: Element | string | null | undefined): boolean {
    ...
    for (;;) {
        ...
        // The row is the last ancestor worth asking - above it sit the containers, whose mocked layout says
        // nothing about visibility - but it is checked itself, or a cell in a hidden row reads as visible.
        if (current.role === 'row') {
            return true;
        }
