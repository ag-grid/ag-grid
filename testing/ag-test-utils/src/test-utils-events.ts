...
/**
 * Dispatches the press the grid's row container is actually listening for: it binds `pointerdown` when the
 * DOM implementation advertises it and `touchstart` when it doesn't, so a test that hard-codes either one
 * silently stops pressing anything where the other applies.
 */
export function fireGridPointerDown(element: HTMLElement, options?: MouseEventInit): void {
    const type = 'onpointerdown' in element ? 'pointerdown' : 'touchstart';
