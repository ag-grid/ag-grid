...
export class FilterDomValidator {
    ...
    private validateColumnFilter(): void {
        ...
        // A virtualised list mounts only the visible rows, so the mounted subset says nothing about the
        // whole selection: `aria-setsize` carries the real total the list is scrolled through.
        const setSize = Number(setList.querySelector('[aria-setsize]')?.getAttribute('aria-setsize'));
        if (setSize > items.length) {
            return;
        }
        let allChecked = true;
