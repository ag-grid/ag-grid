...
export class RowDragDispatcher {
    ...
    public async finish(options: RowDragFinishOptions = {}): Promise<void> {
        ...
        // Detached only after settling: a completing event delivered asynchronously must still reach the
        // recorder that resolves the settle promise, or the caller hangs to the test timeout instead.
        try {
        ...
        if (this.finalDropTarget && this.sourceRowId) {
            // Exactly one: no enter event means the drag lifecycle never ran, which is a failure rather
            // than a reason to skip every assertion below.
            expect(this.rowDragEnterEvents).toHaveLength(1);
            ...
            if (expectedOverId !== this.sourceRowId && expectedOverId !== this.finalDropTarget.getAttribute('row-id')) {
    ...
    /**
     * Best-effort: a drag with no completing event never creates the settle promise, so giving up is a
    ...
    private async waitForSettle(): Promise<void> {
        for (let i = 0; this.settlePromise === undefined && i < 50; ++i) {
            // eslint-disable-next-line no-restricted-syntax -- poll interval for the 100ms budget above, not a guess
            await asyncSetTimeout(2);
        }
        const settle = this.settlePromise;
        if (settle) {
            // Capped too: only a completing event resolves it, so an unbounded await turns a lost rowDragEnd
            // into a test timeout instead of the assertion failure the caller is about to make.
            // eslint-disable-next-line no-restricted-syntax -- the cap on that wait, not a guessed delay
            await Promise.race([settle, asyncSetTimeout(100)]);
        }
    }
