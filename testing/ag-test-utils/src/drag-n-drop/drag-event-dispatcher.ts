    ...
    public async movePointer(targetElement: Element, clientX: number, clientY: number): Promise<void> {
        ...
        if (this.html5DragDrop) {
            ...
            if (targetChanged && previousDropTarget) {
                ...
                // Only when the bubbled event cannot reach it: these events bubble, so a fire on a descendant
                // already notifies the container, and firing again delivers it twice, unlike a real browser.
                if (!dropContainer.contains(previousDropTarget)) {
