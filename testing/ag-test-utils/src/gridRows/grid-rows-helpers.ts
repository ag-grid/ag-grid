...
/** `AG_NO_RETRY=1` drops the retries, so a check fails where it would have silently retried: that is how
 *  you prove a fix for a flaky check is a real wait and not a slightly longer race. */
const RETRY_DELAYS_MS: readonly number[] = process.env.AG_NO_RETRY ? [] : [10, 50, 100];

/** `file:line` of the `check()` call, plus the test name and label when there is one. */
function describeCallSite(target: SnapshotCheckTarget): string {
    const holder: { stack?: string } = {};
    Error.captureStackTrace(holder as Error, target.methodRef);
    const frame = holder.stack?.split('\n')[1]?.match(/\(?([^() ]+\.tsx?:\d+):\d+\)?$/)?.[1];
    const testName = expect.getState().currentTestName;
    return [frame ?? '<unknown>', testName && `in "${testName}"`, target.label && `for "${target.label}"`]
        .filter(Boolean)
        .join(' ');
}
...
export async function runSnapshotCheck(
    ...
    let attempt = target;
    let lastError: any;
    // `makeError` embeds the diagram itself, so only a snapshot mismatch needs one attached below.
    let errorCarriesDiagram = false;

    for (let i = 0; i <= RETRY_DELAYS_MS.length; i++) {
        attempt.loadErrors();
        if (attempt.hasErrors()) {
            lastError = attempt.makeError();
            errorCarriesDiagram = true;
        } else {
            lastError = tryAssertSnapshot(attempt, diagramSnapshot);
            errorCarriesDiagram = false;
        }
        if (!lastError) {
            if (i > 0) {
                // Located, not just labelled: workers interleave on stderr, so the warning lands next to
                // whatever file happens to finish, and most flaky checks carry no label at all.
                process.stderr.write(
                    `${CLASS_NAME_BY_METHOD[target.methodName]} flaky ${target.methodName} at ${describeCallSite(target)} ` +
                        `— passed only after ${RETRY_DELAYS_MS.slice(0, i).reduce((a, b) => a + b, 0)}ms of retries. ` +
                        `Await the state it needs before this check.\n`
                );
            }
            return;
        }
