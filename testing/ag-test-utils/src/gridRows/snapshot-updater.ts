    ...
    function visit(node: any): void {
        if (ts.isCallExpression(node)) {
            ...
            if (
                ...
                // Update mode walks every test file, so without this an unrelated object's multiline
                // `check()` argument is rewritten as though it were a snapshot.
                snapshotReceiver(ts, expr)
            ) {
                ...
                if (ts.isNoSubstitutionTemplateLiteral(arg)) {
                    ...
                    // Only the canonical shape is repaired, where the opening backtick ends its line and the
                    // closing one is alone on the last. Otherwise the slice below drops real content, and
                    // `check(`a\nb`)` becomes an empty snapshot.
                    if (originalLines[0].trim() || originalLines[originalLines.length - 1].trim()) {
                        ts.forEachChild(node, visit);
                        return;
                    }
