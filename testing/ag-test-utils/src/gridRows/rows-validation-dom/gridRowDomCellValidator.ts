...
import { isInNestedGrid } from './containers-helpers';
...
export class GridRowDomCellValidator {
    ...
    public constructor(private readonly gridRows: GridRows) {
        ...
        // Behind the same guard as the reads above: `getColumn`/`getAllGridColumns` belong to ColumnApiModule,
        // so an unguarded lookup logs error 200 twice on a grid that omits it.
        this.autoGroupColumn = hasColumnApi ? this.lookupAutoGroupColumn() : undefined;
    ...
    private collectRowSpanCoverage(): Map<string, Set<string>> {
        ...
        for (const cellNode of Array.from(spannedCells)) {
            if (isInNestedGrid(cellNode as HTMLElement, rootEl)) {
                continue;
            }
    ...
    private validateCell(
        ...
        // Custom renderers may wrap the value; tolerate that, but flag when the value is missing entirely
        // (an empty cellValue + non-empty text is fine — renderer-controlled). A registered string renderer
        // is as free to reshape the text as a function one; only the grid's own `ag*` ones are predictable.
        const isCustomRenderer =
            typeof cellRenderer === 'function' || (typeof cellRenderer === 'string' && !cellRenderer.startsWith('ag'));
        if (isCustomRenderer && (!stringCellValue || textContent.includes(stringCellValue))) {
            return;
        }
