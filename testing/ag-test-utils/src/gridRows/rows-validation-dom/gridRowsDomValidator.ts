...
import { getGridHTMLElement, getGridRowsHtmlElementsById } from '../gridHtmlRows';
...
export class GridRowsDomValidator {
    ...
    public validate(gridRows: GridRows): void {
        ...
        // Once per pass, not once per row — and a local, because a detail grid recurses into `validate`.
        const rowElementsById = getGridRowsHtmlElementsById(gridRows.api);
        ...
        for (const row of displayedRows) {
            ...
            const rowElements = this.resolveRowElements(rowElementsById, row, rowVirtualisationActive);
            if (!rowElements) {
                continue;
            }
    ...
    private validateRowCommon(
        ...
        rowElementsById: ReadonlyMap<string, HTMLElement[]>
    ): void {
        const rowElements = this.resolveRowElements(rowElementsById, row);
        if (!rowElements) {
            return;
        }
        this.runRowValidators(
    ...
    private resolveRowElements(
        ...
        const rowElements = rowElementsById.get(stringId) ?? [];
        if (!allowMissing) {
            this.errors.add(
                row,
                !rowElements.length &&
