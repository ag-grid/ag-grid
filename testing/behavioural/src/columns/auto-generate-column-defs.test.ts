    ...
    describe('processAutoGeneratedColumnDefs callback', () => {
        ...
        test('callback can filter out columns', async () => {
            const api = createGrid({
                ...
                rowData: [{ a: 1, b: 2, c: 3 }],
                processAutoGeneratedColumnDefs: (params) => {
                    return params.columnDefs.filter((colDef) => colDef.field !== 'b');
                },
            });
        ...
        test('callback returning non-array logs warning and uses original defs', async () => {
        ...
        test('callback receives correct params', () => {
            ...
            let receivedParams: { columnDefs: ColDef[]; rowData: any[] } | undefined;
