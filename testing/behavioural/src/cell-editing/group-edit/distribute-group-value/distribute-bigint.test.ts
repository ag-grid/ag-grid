...
describe('bigint distribution', () => {
    ...
    test('bigint uniform with min constraint', async () => {
        const api = await createBigIntGrid(
            'bigint-min',
            [
            ...
            {
                groupRowValueSetter: (params) => distributeGroupValue(params, { distribution: 'uniform', min: 15n }),
            }
        );

        // 20 / 2 = 10 each, but min=15 → both clamped to 15
        const group = api.getRowNode('row-group-region-R-country-C')!;
        group.setDataValue('amount', 20n, 'ui');
        await asyncSetTimeout(0);

        expect(api.getRowNode('a1')?.data?.amount).toBe(15n);
        expect(api.getRowNode('a2')?.data?.amount).toBe(15n);
    });

    test('bigint uniform with max constraint', async () => {
        const api = await createBigIntGrid(
            'bigint-max',
            [
                { id: 'a1', region: 'R', country: 'C', amount: 10n },
            ...
            {
                groupRowValueSetter: (params) => distributeGroupValue(params, { distribution: 'uniform', max: 30n }),
            }
        );

        // 100 / 2 = 50 each, max=30 → both clamped to 30
        const group = api.getRowNode('row-group-region-R-country-C')!;
        group.setDataValue('amount', 100n, 'ui');
        await asyncSetTimeout(0);

        expect(api.getRowNode('a1')?.data?.amount).toBe(30n);
        expect(api.getRowNode('a2')?.data?.amount).toBe(30n);
    });
...
describe('bigint edge cases', () => {
    ...
    test('bigint overwrite with in-range value and min/max returns bigint', async () => {
        const api = await createBigIntGrid(
            'bigint-overwrite-clamp-inrange',
            [
                { id: 'a1', region: 'R', country: 'C', amount: 10n },
                { id: 'a2', region: 'R', country: 'C', amount: 20n },
            ],
            {
                groupRowValueSetter: (params) =>
                    distributeGroupValue(params, { distribution: 'overwrite', min: 0n, max: 100n }),
            }
        );

        // 5n is within [0, 100] — clampValue returns the normalized bigint (not the original input)
        const group = api.getRowNode('row-group-region-R-country-C')!;
        group.setDataValue('amount', 5n, 'ui');
        await asyncSetTimeout(0);

        expect(api.getRowNode('a1')?.data?.amount).toBe(5n);
        expect(api.getRowNode('a2')?.data?.amount).toBe(5n);
    });

    test('bigint overwrite clamped to min', async () => {
        const api = await createBigIntGrid(
            'bigint-overwrite-clamp-min',
            [
                { id: 'a1', region: 'R', country: 'C', amount: 10n },
                { id: 'a2', region: 'R', country: 'C', amount: 20n },
            ],
            {
                groupRowValueSetter: (params) => distributeGroupValue(params, { distribution: 'overwrite', min: 10n }),
            }
        );

        // 3n below min=10 — clamped to 10n
        const group = api.getRowNode('row-group-region-R-country-C')!;
        group.setDataValue('amount', 3n, 'ui');
        await asyncSetTimeout(0);

        expect(api.getRowNode('a1')?.data?.amount).toBe(10n);
        expect(api.getRowNode('a2')?.data?.amount).toBe(10n);
    });

    test('bigint overwrite clamped to max', async () => {
        const api = await createBigIntGrid(
            'bigint-overwrite-clamp-max',
            [
                { id: 'a1', region: 'R', country: 'C', amount: 10n },
                { id: 'a2', region: 'R', country: 'C', amount: 20n },
            ],
            {
                groupRowValueSetter: (params) => distributeGroupValue(params, { distribution: 'overwrite', max: 2n }),
            }
        );

        // 5n above max=2 — clamped to 2n
        const group = api.getRowNode('row-group-region-R-country-C')!;
        group.setDataValue('amount', 5n, 'ui');
        await asyncSetTimeout(0);

        expect(api.getRowNode('a1')?.data?.amount).toBe(2n);
        expect(api.getRowNode('a2')?.data?.amount).toBe(2n);
    });

    test('bigint first strategy with min/max clamps the value', async () => {
        const api = await createBigIntGrid(
            'bigint-first-clamp',
            [
                { id: 'a1', region: 'R', country: 'C', amount: 10n },
            ...
            {
                aggFunc: 'first',
                groupRowValueSetter: (params) => distributeGroupValue(params, { min: 5n, max: 15n }),
            }
        );

        // first strategy writes to first child only, clamped to [5, 15]
        const group = api.getRowNode('row-group-region-R-country-C')!;
        group.setDataValue('amount', 100n, 'ui');
        await asyncSetTimeout(0);

        expect(api.getRowNode('a1')?.data?.amount).toBe(15n);
        expect(api.getRowNode('a2')?.data?.amount).toBe(20n); // unchanged
    });
