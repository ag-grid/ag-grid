...
// ── Grouped data — shared by the grouping-pipelines benches. ─────────────────────────────────────
// Five group levels are always populated; a suite groups by however many it needs (the rest are
// inert extra fields). `name` feeds filter benches, `value`/`count` feed aggregation.
...
/** Structural churn: delete ~10%, regroup ~18% (new group keys), add ~13% — changes the tree shape. */
export function buildGroupedStructuralUpdate(rows: GroupedRow[], seed = 0x3d24a75): GroupedRow[] {
    ...
    const deletes = Math.floor(count * 0.1);
    for (let i = 0; i < deletes; ++i) {
        result.splice(prng.nextInt(0, count - 1), 1);
        --count;
    }

    const moves = Math.floor(count * 0.18);
    ...
    const adds = Math.floor(count * 0.13);
...
// ── Wide value-column rows (v0..vN) — shared by the aggregation / pivot benches. ──────────────────
