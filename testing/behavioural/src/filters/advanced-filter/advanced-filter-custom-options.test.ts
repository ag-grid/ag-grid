...
describe('Advanced Filter — custom filter options', () => {
    ...
    test('an operand the option does not take is dropped, so a column change cannot resurrect it', async () => {
        const nearly = (numberOfInputs: 1 | 2): IFilterOptionDef => ({
            displayKey: 'nearly',
            displayName: 'Nearly',
            numberOfInputs,
            predicate: ([from, to], cellValue) =>
        ...
        const api = await gridsManager.createGridAndWait('grid1', {
            columnDefs: [
                { field: 'age', filter: 'agNumberColumnFilter', filterParams: { filterOptions: [nearly(1)] } },
                { field: 'score', filter: 'agNumberColumnFilter', filterParams: { filterOptions: [nearly(2)] } },
            ],
            rowData: [
                { age: 25, score: 10 },
                { age: 40, score: 90 },
        ...
        // The model holds what its option takes and no more, which is what a column offering the same key
        // with a second value would otherwise inherit.
        expect(api.getAdvancedFilterModel()).toEqual({
            filterType: 'number',
            colId: 'age',
            type: 'nearly',
        ...
        const builder = await AdvancedFilterBuilderHarness.open(api);
        const [item] = await builder.conditionItems();
        expect(builder.valuePills(item).map((pill) => pill.textContent?.trim())).toEqual(['25']);
