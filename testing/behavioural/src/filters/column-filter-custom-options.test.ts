...
import type { ColDef, GridApi, GridOptions, IFilterOptionDef } from 'ag-grid-community';
import {
...
describe('Column filter — custom options taking 0, 1 or 2 values', () => {
    ...
    test('a text `inRange` listed explicitly is not range-validated either', async () => {
        // Deliberate: the Text Filter does not evaluate `inRange`, which triggers warning #327.
...
describe('Column filter — a model missing values the option needs', () => {
    ...
    ] as const)('a %s option short of a value filters nothing rather than everything', async (_name, filter, value) => {
        const api: GridApi<Row> = await gridsManager.createGridAndWait('grid1', {
...
describe('Column filter — a built-in option the filter cannot evaluate', () => {
    ...
    test('an option belonging to another filter type is reported by name', async () => {
        enableDevValidations({ throwOn: ALL_SEVERITIES, suppress: [327] });
        const warnSpy = vi.spyOn(console, 'warn').mockImplementation(() => {});
    ...
    test('a relative date range on a text filter is reported', async () => {
        enableDevValidations({ throwOn: ALL_SEVERITIES, suppress: [327] });
        const warnSpy = vi.spyOn(console, 'warn').mockImplementation(() => {});
    ...
    test('a Custom Filter Option supplying the same key is not reported, as it defines the evaluation', async () => {
        const warnSpy = vi.spyOn(console, 'warn').mockImplementation(() => {});
    ...
    test("the filter's own options are not reported", async () => {
        const warnSpy = vi.spyOn(console, 'warn').mockImplementation(() => {});

        const api: GridApi = await createGrid(['contains', 'notContains', 'equals', 'startsWith', 'blank', 'notBlank']);
        await ColumnFilterHarness.open(api, 'athlete');
...
describe('Column filter — a `defaultOption` the dropdown does not offer', () => {
    ...
    test('is reported and falls back to the first offered option', async () => {
        // Deliberate: `lessThan` is evaluated by the Number Filter but is not one of the offered options.
        enableDevValidations({ throwOn: ALL_SEVERITIES, suppress: [328] });
        const warnSpy = vi.spyOn(console, 'warn').mockImplementation(() => {});
        ...
        const filter = await ColumnFilterHarness.open(api, 'age');
        expect(warnSpy.mock.calls.flat().join(' ')).toContain('warning #328');
