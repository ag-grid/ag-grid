...
(function () {
    function init() {
        var mount = document.getElementById('live-schedulerpro-demo');
        if (!mount) return;
        if (!window.bryntum || !window.bryntum.schedulerpro) return;
        if (mount.dataset.bryntumInitialized === '1') return;
        mount.dataset.bryntumInitialized = '1';

        const { SchedulerPro, Toast } = window.bryntum.schedulerpro;
