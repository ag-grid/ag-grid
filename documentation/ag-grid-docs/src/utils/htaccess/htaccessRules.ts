...
${SITE_SINGLE_HOP_REWRITES.map((r) => {
    ...
    # SE-91/SE-86: blog.ag-grid.com -> www.ag-grid.com/blog/ . Host-scoped, so none of
    # this affects www or apex. ORDER MATTERS: every specific rule must precede the
    # catch-all host swap at the end, or the catch-all wins and the specific target is
    # never reached. All targets are FINAL destinations -- www, never the apex, and never
    # an intermediate slug -- because SE-86 requires a single hop.
    #
    # Verified 2026-08-20: both sitemaps carry the same 291 URLs with identical paths, so
    # the catch-all covers the whole sitemap set and no per-post rows are needed. The
    # explicit rules below are only the cases the catch-all would get wrong.

    # 410, NOT a redirect. /whats-new-in-ag-grid-v24/ is the target of a spam campaign
    # (5,869 inbound links flagged spam). 301ing it would carry those signals onto the
    # main domain, so it is declared gone. Must precede the catch-all.
    RewriteCond %{HTTP_HOST} ^blog\\.ag-grid\\.com$ [NC]
    RewriteRule ^/?whats-new-in-ag-grid-v24/?$ - [R=410,L]

    # Ghost's own redirects.json holds 12 slug redirects that still run on the new
    # instance. Host-swapping these sources would land on /blog/<old-slug>, where Ghost
    # would redirect again -- two hops, silently. So each maps straight to its FINAL slug.
    # Three of the twelve are in the sitemap and already in the SE-91 map; these are the
    # nine that are not, so nothing else surfaces them.
    RewriteCond %{HTTP_HOST} ^blog\\.ag-grid\\.com$ [NC]
    RewriteRule ^/?tag/react/?$ https://www.ag-grid.com/blog/tag/react-data-grid/ [R=301,L]
    RewriteCond %{HTTP_HOST} ^blog\\.ag-grid\\.com$ [NC]
    RewriteRule ^/?conferences-ag-grid-amsterdam-june-2022/?$ https://www.ag-grid.com/blog/js-nation-and-react-summit-june-2022-overview/ [R=301,L]
    RewriteCond %{HTTP_HOST} ^blog\\.ag-grid\\.com$ [NC]
    RewriteRule ^/?getting-more-from-your-datagrid-introducing-adaptable/?$ https://www.ag-grid.com/blog/adaptable-tools-demo-and-interview/ [R=301,L]
    RewriteCond %{HTTP_HOST} ^blog\\.ag-grid\\.com$ [NC]
    RewriteRule ^/?javascript-grid-comparison-column-pinning-ag-grid/?$ https://www.ag-grid.com/blog/heres-why-column-pinning-in-react-datagrid-by-ag-grid-wins-over-competition/ [R=301,L]
    RewriteCond %{HTTP_HOST} ^blog\\.ag-grid\\.com$ [NC]
    RewriteRule ^/?whats-new-in-ag-studio-2/?$ https://www.ag-grid.com/blog/whats-new-in-ag-studio-2-0-javascript-embedded-analytics/ [R=301,L]
    RewriteCond %{HTTP_HOST} ^blog\\.ag-grid\\.com$ [NC]
    ...
    RewriteCond %{HTTP_HOST} ^blog\\.ag-grid\\.com$ [NC]
    RewriteRule ^/?angular-2-0-web-components-and-ag-grid/?$ https://www.ag-grid.com/angular-data-grid/getting-started/ [R=301,L]

    # The /amp/ form is an explicit redirects.json source AND would match the generic
    # /amp/ rule below. It is listed first so the explicit target wins; otherwise the
    # generic rule strips /amp/ and hands off to Ghost for a second hop.
    RewriteCond %{HTTP_HOST} ^blog\\.ag-grid\\.com$ [NC]
    RewriteRule ^/?integrated-charts-community-vs-enterprise/amp/?$ https://www.ag-grid.com/blog/enhancing-ag-grid-enterprise-with-ag-charts-enterprise/ [R=301,L]
    RewriteCond %{HTTP_HOST} ^blog\\.ag-grid\\.com$ [NC]
    RewriteRule ^/?integrated-charts-community-vs-enterprise/?$ https://www.ag-grid.com/blog/enhancing-ag-grid-enterprise-with-ag-charts-enterprise/ [R=301,L]

    # Off-sitemap, from backlink data. The Vuestic tutorial is handed back to its authors
    # at Epicmax by decision 2026-08-20 -- note this sends the link equity off-domain and
    # depends on their URL surviving. Our own copy stays live but unreachable from here.
    RewriteCond %{HTTP_HOST} ^blog\\.ag-grid\\.com$ [NC]
    RewriteRule ^/?vuestic-ui-app-with-ag-grid-tutorial/?$ https://epicmax.co/blog/vuestic-ui-with-ag-grid [R=301,L]

    # Legacy pre-Ghost WordPress permalink for the React Fiber article -- the single
    # highest-value genuine backlink target in the set. Matched by prefix because the
    # dated path is long and only its stem is documented.
    RewriteCond %{HTTP_HOST} ^blog\\.ag-grid\\.com$ [NC]
    RewriteRule ^/?index\\.php/2018/11/29/inside-fiber https://www.ag-grid.com/blog/inside-fiber-an-in-depth-overview-of-the-new-reconciliation-algorithm-in-react/ [R=301,L]

    # 410, decided 2026-08-20. Off-sitemap, and already 404 on both hosts -- the post is
    # long gone, so there is nothing to redirect to. Declaring it Gone tells crawlers to
    # drop it rather than leaving them to infer that from a soft 404, and it beats the
    # catch-all, which would otherwise land visitors on a /blog/ 404.
    RewriteCond %{HTTP_HOST} ^blog\\.ag-grid\\.com$ [NC]
    RewriteRule ^/?ag-grid-vs-datatables/?$ - [R=410,L]

    # Legacy /amp/ paths: AMP was never enabled and these are not advertised, but they
    # still attract links. Strip /amp/ and send to the post rather than a dead
    # /blog/<post>/amp/. Must follow the explicit /amp/ rule above.
    RewriteCond %{HTTP_HOST} ^blog\\.ag-grid\\.com$ [NC]
    RewriteRule ^/?(.+?)/amp/?$ https://www.ag-grid.com/blog/$1/ [R=301,L]

    # Hotlinked images. 21 are known from backlink data but the paths were never
    # enumerated; verified 2026-08-20 that content paths are preserved byte-for-byte under
    # /blog/, so one pattern covers all of them plus any hotlink nobody found.
    RewriteCond %{HTTP_HOST} ^blog\\.ag-grid\\.com$ [NC]
    RewriteRule ^/?content/(.*)$ https://www.ag-grid.com/blog/content/$1 [R=301,L]

    # Pagination, feeds and the sitemap: live URLs that appear in no sitemap, so the map
    # never surfaced them. Paths are identical under /blog/, so they only need the swap.
    RewriteCond %{HTTP_HOST} ^blog\\.ag-grid\\.com$ [NC]
    RewriteRule ^/?(page/[0-9]+|(?:tag|author)/[^/]+/page/[0-9]+)/?$ https://www.ag-grid.com/blog/$1/ [R=301,L]
    RewriteCond %{HTTP_HOST} ^blog\\.ag-grid\\.com$ [NC]
    RewriteRule ^/?(rss|feed|(?:tag|author)/[^/]+/rss)/?$ https://www.ag-grid.com/blog/$1/ [R=301,L]
    RewriteCond %{HTTP_HOST} ^blog\\.ag-grid\\.com$ [NC]
    RewriteRule ^/?(sitemap[^/]*\\.xml)$ https://www.ag-grid.com/blog/$1 [R=301,L]

    # Catch-all host swap. LAST, so everything above wins. Covers all 291 sitemap URLs
    # (posts, tags, authors, pages) plus anything not enumerated, in one rule that cannot
    # go stale when a post is published -- which is exactly how the 282-row map fell nine
    # URLs behind the live 291.
    RewriteCond %{HTTP_HOST} ^blog\\.ag-grid\\.com$ [NC]
    RewriteRule ^/?(.*)$ https://www.ag-grid.com/blog/$1 [R=301,L]
