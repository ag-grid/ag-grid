...
test.agExample(import.meta, () => {
    test.eachFramework('Example', async ({ page, agIdFor }) => {
        ...
        await expect(firstRowPrice.locator('xpath=ancestor::div[@role="row"]')).toHaveAttribute('row-index', '0');
        await expect(secondRowPrice.locator('xpath=ancestor::div[@role="row"]')).toHaveAttribute('row-index', '1');
        await expect(thirdRowPrice.locator('xpath=ancestor::div[@role="row"]')).toHaveAttribute('row-index', '2');
