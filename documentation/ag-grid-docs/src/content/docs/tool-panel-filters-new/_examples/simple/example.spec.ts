...
test.agExample(import.meta, () => {
    test.eachFramework('Example', async ({ page, agIdFor }) => {
        ...
        await expect(firstCell.locator('xpath=ancestor::div[@role="row"]')).toHaveAttribute('row-index', '0');
        ...
        await expect(agIdFor.cell('0', 'athlete').first().locator('xpath=ancestor::div[@role="row"]')).toHaveAttribute(
            'row-index',
            '0'
        );
    });
