...
// Each button handler reaches for `gridApi` itself rather than delegating to a shared
// `setColumnCount` / `setRowCount`. The framework examples are generated by lifting the functions
// the HTML calls into the component, where the grid api is in scope; a helper one level down stays
// at module scope and its `gridApi` reference resolves to nothing, which left every one of these
// buttons throwing `gridApi is not defined` in React, Angular and Vue.
