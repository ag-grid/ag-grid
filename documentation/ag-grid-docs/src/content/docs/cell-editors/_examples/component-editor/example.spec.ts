...
const runTests = (frameworkTest: any) => {
    ...
    frameworkTest(
        ...
        // React's warning reaches the console in two shapes depending on the build: with the
        // specifier already substituted ('for the `value` attribute') and with it left in place
        // and the argument appended ('for the `%s` attribute ... value'). Match the common prefix
        // so either shape is allowed.
        { allowedConsoleMessages: ['Received NaN for the'] }
    );
    ...
    frameworkTest('Backspace starts the editor cleared', async ({ agIdFor, page }: any) => {
        test.skip(!backspaceReachesTheGrid(), 'WebKit navigates back on Backspace');
