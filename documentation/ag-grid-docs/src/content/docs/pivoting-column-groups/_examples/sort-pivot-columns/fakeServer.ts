...
export function FakeServer(allData) {
    ...
    function getPivotFields(request) {
        ...
        // Return pivot fields in natural (generated) order, not pre-sorted, so the grid's
        // pivotSort (asc/desc/natural) is observable rather than masked by a server-side sort.
        const template = "SELECT DISTINCT ({0} + '_{1}') AS {0} FROM ?";
