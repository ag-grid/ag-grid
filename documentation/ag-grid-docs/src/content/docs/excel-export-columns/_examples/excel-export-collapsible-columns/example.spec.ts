...
test.agExample(import.meta, () => {
    test.eachFramework('Closed Sports Results group shows only the total column', async ({ agIdFor, page }) => {
        await ensureGridReady(page);
        await waitForGridContent(page);

        // Athlete details are always shown.
        await expect(agIdFor.cell('0', 'athlete')).toContainText('Michael Phelps');
        await expect(agIdFor.cell('0', 'country')).toContainText('United States');

        // Sports Results is closed: total (columnGroupShow 'closed') is visible, medals ('open') are hidden.
    ...
    test.eachFramework('Opening the group reveals the medal columns', async ({ agIdFor, page }) => {
        ...
        // Medal columns become visible, total column is hidden.
