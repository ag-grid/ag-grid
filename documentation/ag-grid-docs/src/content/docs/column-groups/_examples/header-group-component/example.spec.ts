...
test.agExample(import.meta, () => {
    test.eachFramework('Custom header group renders its label and expand button', async ({ agIdFor, page }) => {
        ...
        await expect(athleteGroup.locator('.customExpandButton')).toHaveClass(/collapsed/);
    ...
    test.eachFramework('Custom expand button opens the group', async ({ agIdFor, page }) => {
        ...
        await expect(athleteGroup.locator('.customExpandButton')).toHaveClass(/expanded/);
