...
test.agExample(import.meta, () => {
    test.eachFramework('lockPinned athlete stays pinned left', async ({ page, agIdFor }) => {
        ...
        const pinnedLeftHeader = page.locator('.ag-header-row-column .ag-grid-pinned-left-cells');
    ...
    test.eachFramework('lockPinned age stays in center (not pinnable)', async ({ page, agIdFor }) => {
        ...
        const centerHeader = page.locator('.ag-header-row-column .ag-grid-scrolling-cells');
    ...
    test.eachFramework('locked cells have lock-pinned class', async ({ page }) => {
        ...
        const athleteCell = page.locator(
            '.ag-grid-scrolling-container .ag-row:first-child .ag-grid-pinned-left-cells .ag-cell[col-id="athlete"]'
        );
        await expect(athleteCell).toHaveClass(/lock-pinned/);

        // First row age cell should have lock-pinned class
        const ageCell = page.locator(
            '.ag-grid-scrolling-container .ag-row:first-child .ag-grid-scrolling-cells .ag-cell[col-id="age"]'
        );
