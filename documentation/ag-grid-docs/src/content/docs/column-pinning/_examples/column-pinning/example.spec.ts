...
test.agExample(import.meta, () => {
    test.eachFramework('initial pinned layout', async ({ page }) => {
        ...
        const leftHeaders = await getPinnedHeaderColIds(page, '.ag-header-row-column .ag-grid-pinned-left-cells');
        expect(leftHeaders).toEqual(['rowNum', 'athlete', 'age']);

        const rightHeaders = await getPinnedHeaderColIds(page, '.ag-header-row-column .ag-grid-pinned-right-cells');
    ...
    test.describe('sizeColumnsToFit with pinned columns', () => {
        ...
        test.vanilla(
            ...
            async ({ page, remoteGrid }) => {
                ...
                const headersBefore = await getPinnedHeaderColIds(
                    page,
                    '.ag-header-row-column .ag-grid-pinned-left-cells'
                );
                ...
                const headersAfter = await getPinnedHeaderColIds(
                    page,
                    '.ag-header-row-column .ag-grid-pinned-left-cells'
                );
        ...
        test.vanilla(
            ...
            async ({ page, remoteGrid }) => {
                ...
                const headersBefore = await getPinnedHeaderColIds(
                    page,
                    '.ag-header-row-column .ag-grid-pinned-right-cells'
                );
        ...
        test.vanilla(
            ...
            async ({ page, remoteGrid }) => {
                ...
                const leftBefore = await getPinnedHeaderColIds(
                    page,
                    '.ag-header-row-column .ag-grid-pinned-left-cells'
                );
                expect(leftBefore).toEqual(['athlete', 'age']);
                const rightBefore = await getPinnedHeaderColIds(
                    page,
                    '.ag-header-row-column .ag-grid-pinned-right-cells'
                );
                ...
                const leftAfter = await getPinnedHeaderColIds(page, '.ag-header-row-column .ag-grid-pinned-left-cells');
                expect(leftAfter).toEqual(['athlete', 'age']);
                const rightAfter = await getPinnedHeaderColIds(
                    page,
                    '.ag-header-row-column .ag-grid-pinned-right-cells'
                );
