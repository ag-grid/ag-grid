import type { TransitionFacts, VersionChangelog } from '@ag-website-shared/changes/change-types';
...
//
const suppressAdvancedFilterEval: TransitionFacts = {
    oldApi: '`suppressAdvancedFilterEval`',
    // the pre-v34 mechanism was `new Function()`, not literally `eval()` — same CSP
    // implication, but the option's job was to avoid the dynamic-evaluation path
    oldDescription:
        "a grid option that made the Advanced Filter avoid dynamic code evaluation (`new Function()`, which requires the CSP `script-src 'unsafe-eval'` allowance); from v34 the Advanced Filter never uses function evaluation, so the option has no effect",
    newApi: 'no replacement (simply remove the option)',
    detectWords: 'suppressAdvancedFilterEval',
    mitigation:
        "Remove the option. The Advanced Filter does not use dynamic code evaluation, so the `script-src 'unsafe-eval'` Content Security Policy allowance can also be removed if no other grid features rely on it.",
};
// ### Filters
//
// - `ISetFilter` interface is deprecated. When retrieving a Set Filter via `api.getColumnFilterInstance(column)`, use `SetFilterUi`. To access the methods that were previously available on `ISetFilter` (but not on `SetFilterUi`), instead retrieve the Set Filter Handler via `api.getColumnFilterHandler(column)` and use the `SetFilterHandler` interface. See [Set Filter API](./filter-set-api/#set-filter-api) for more information.
//
const iSetFilter: TransitionFacts = {
    oldApi: 'the `ISetFilter` interface',
    newApi: '`SetFilterUi` and `SetFilterHandler`',
    detectWords: 'ISetFilter',
    mitigation:
        'When retrieving a Set Filter via `api.getColumnFilterInstance(column)`, use `SetFilterUi`. To access the methods that were previously available on `ISetFilter` but not on `SetFilterUi`, retrieve the Set Filter Handler via `api.getColumnFilterHandler(column)` and use the `SetFilterHandler` interface. See [Set Filter API](https://www.ag-grid.com/javascript-data-grid/filter-set-api/#set-filter-api).',
};
...
// {% /expandingSection %}
//
const filterInstanceGetSetModel: TransitionFacts = {
    oldApi: '`getModel()` and `setModel()` on grid-provided filter instances',
    newApi: '`api.getColumnFilterModel(column)` and `api.setColumnFilterModel(column)`',
    detectWords: ['getModel', 'setModel', 'getColumnFilterInstance', 'getFilterInstance'],
    mitigation:
        'Use `api.getColumnFilterModel(column)` and `api.setColumnFilterModel(column)` instead of retrieving a filter instance and calling `getModel()`/`setModel()` on it. Custom filter components can migrate to Filter Handlers, which split filter logic out of the UI component — see [Custom Filter Components](https://www.ag-grid.com/javascript-data-grid/component-filter/).',
};

export const v34: VersionChangelog = {
    deprecations: {
        suppressAdvancedFilterEval,
        iSetFilter,
        filterInstanceGetSetModel,
    },
};
