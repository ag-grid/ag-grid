...
- **Per-framework advice.** `mitigation` is `string | MitigationAdvice[] | null`, where a `MitigationAdvice` line is either a plain `string` (non-framework-dependent, shown to everyone) or a `{ frameworks, content }` object (scoped to an explicit list). A top-level plain string is shorthand for one non-framework-dependent line. Use the array form only when the advice genuinely differs by framework — the docs page's `isFramework(...)` blocks are the signal. Rules:
    - A plain-string line applies to all frameworks (including vanilla `'javascript'`); an object line lists the exact frameworks it applies to (`'react' | 'angular' | 'vue' | 'javascript'`, `frameworks` required).
    - **Every non-null mitigation must include at least one line that is non-framework-dependent (a plain string) or lists all four frameworks** — otherwise some framework, typically vanilla `'javascript'`, would get no advice. Validation rejects a mitigation that only has partial per-framework lines.
    - Entries are **additive**: a reader on framework X sees every plain-string line plus every object line whose `frameworks` includes X. Put shared advice in one plain-string line and framework-specific extras in their own object lines — do not repeat the shared text in each.
    - Frameworks include **vanilla JavaScript**, and a framework app can use the vanilla `createGrid` API / vanilla components. So advice about the class-style (`IFilterComp`-type) contract typically scopes to `['javascript', 'angular', 'vue']`, while React (hook/prop-based) advice scopes to `['react']`; anything common stays a plain string.
    - In the compiled output a plain-string line becomes `{ frameworks: null, content }` (`null` = non-framework-dependent).
