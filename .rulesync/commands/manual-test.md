...
Docs URLs follow the pattern: `https://ag-grid.com/javascript-data-grid/{pageName}/#{fragment}`

The URL may use any framework prefix (`javascript-data-grid`, `react-data-grid`, `angular-data-grid`, `vue-data-grid`) — ignore the framework, we always read the TypeScript source.

## Step 1: Parse input and locate example source
...
4. If a fragment was provided:
   ...
   - Extract the `title` attribute value — this is the `exampleName`
...
## Step 3: Update `src/gridOptions.ts`

The template's `src/gridOptions.ts` has a typed structure that must be preserved:
