...
8. See Also — use very rarely, if at all. Only add a See Also section when linking to a page in a **different nav section** that is directly relevant and not easily discoverable from the sidebar. Never use See Also to link to child or sibling pages within the same nav section — those are already visible in the sidebar. If all candidate links are within the same section, or the target page is easily found through navigation, omit See Also entirely. When in doubt, leave it out. If deemed a good addition, consider in-line linking from an exsiting or new text instead of See Also.
...
Use `{% apiDocumentation %}` sparingly and inline — place it **immediately after the relevant code snippet** (configuration block) within the section it relates to. Do not create a standalone "API reference" section at the end of a page.

**Rules:**

-   **Max 2 `names` entries** per `{% apiDocumentation %}` block.
-   **Object/complex types only** — use `{% apiDocumentation %}` only for members whose values are objects, interfaces, or callback signatures where the shape needs explaining. Do not use it for members that accept only a basic boolean, string, or numeric value; explain those inline in prose or code snippet comments instead.
