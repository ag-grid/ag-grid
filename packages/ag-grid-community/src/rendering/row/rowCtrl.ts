...
export class RowCtrl extends BeanStub<RowCtrlEvent> {
    ...
    private initialiseRowComp(gui: RowGui): void {
        ...
        initialRowClasses.forEach((name) => comp.updateCss(name, true));
        ...
        if (this.useAnimationFrameForCreate) {
            ...
            this.beans.animationFrameSvc!.addDestroyTask(() => {
                ...
                gui.rowComp.updateCss('ag-after-created', true);
            });
        }
    ...
    private executeSlideAndFadeAnimations(gui: RowGui): void {
        ...
        if (shouldFade) {
            _batchCall(() => {
                gui.rowComp.updateCss('ag-opacity-zero', false);
            });
    ...
    private onRowPinned(): void {
        for (const gui of this.allRowGuis) {
            gui.rowComp.updateCss('ag-row-pinned-target', !!this.rowNode.pinnedSibling);
        }
    }
    ...
    private onRowNodeHighlightChanged(): void {
        ...
        this.allRowGuis.forEach((gui) => {
            ...
            gui.rowComp.updateCss('ag-row-highlight-above', aboveOn);
            gui.rowComp.updateCss('ag-row-highlight-below', belowOn);
        });
    }

    private postProcessRowDragging(): void {
        const dragging = this.rowNode.dragging;
        this.allRowGuis.forEach((gui) => gui.rowComp.updateCss('ag-row-dragging', dragging));
    }
    ...
    private onUiLevelChanged(): void {
        ...
        if (this.rowLevel != newLevel) {
            ...
            this.allRowGuis.forEach((gui) => {
                gui.rowComp.updateCss(classToAdd, true);
                gui.rowComp.updateCss(classToRemove, false);
            });
        }
    ...
    protected refreshFirstAndLastRowStyles(): void {
        ...
        if (this.firstRowOnPage !== newFirst) {
            ...
            this.allRowGuis.forEach((gui) => gui.rowComp.updateCss('ag-row-first', newFirst));
        }
        if (this.lastRowOnPage !== newLast) {
            this.lastRowOnPage = newLast;
            this.allRowGuis.forEach((gui) => gui.rowComp.updateCss('ag-row-last', newLast));
        }
    }
    ...
    private postProcessClassesFromGridOptions(): void {
        ...
        cssClasses.forEach((classStr) => {
            this.allRowGuis.forEach((c) => c.rowComp.updateCss(classStr, true));
        });
    }
    ...
    private postProcessRowClassRules(): void {
        this.beans.rowStyleSvc?.processRowClassRules(
            this.rowNode,
            (className: string) => {
                this.allRowGuis.forEach((gui) => gui.rowComp.updateCss(className, true));
            },
            (className: string) => {
                this.allRowGuis.forEach((gui) => gui.rowComp.updateCss(className, false));
            }
        );
    }
    ...
    public destroyFirstPass(suppressAnimation: boolean = false): void {
        ...
        if (!suppressAnimation && _isAnimateRows(this.gos) && !rowNode.sticky) {
            ...
            if (rowStillVisibleJustNotInViewport) {
                ...
                this.allRowGuis.forEach((gui) => gui.rowComp.updateCss('ag-opacity-zero', true));
            }
        }
    ...
    private setFocusedClasses(gui?: RowGui): void {
        this.forEachGui(gui, (gui) => {
            gui.rowComp.updateCss('ag-row-focus', this.rowFocused);
            gui.rowComp.updateCss('ag-row-no-focus', !this.rowFocused);
        });
    }
    ...
    private updateRowIndexes(gui?: RowGui): void {
        ...
        this.forEachGui(gui, (c) => {
            ...
            c.rowComp.updateCss('ag-row-even', rowIsEven);
            c.rowComp.updateCss('ag-row-odd', !rowIsEven);
