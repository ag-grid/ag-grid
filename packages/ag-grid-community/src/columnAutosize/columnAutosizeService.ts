...
export class ColumnAutosizeService extends BeanStub implements NamedBean {
    ...
    public postConstruct(): void {
        ...
        this.setupStrategyEventListeners(autoSizeStrategy?.events);
    }

    private setupStrategyEventListeners(events: AgPublicEventType[] | undefined): void {
        if (!events?.length) {
            return;
        }
    ...
    private applyStrategy(strategy: AutoSizeStrategy): void {
        ...
        if (type === 'fitGridWidth') {
            ...
            this.sizeColumnsToFit(strategy.width, 'sizeColumnsToFit');
        } else {
    ...
    public applyAutosizeStrategy(): void {
        ...
        this.applyStrategyWhenSettled(autoSizeStrategy);
    }
