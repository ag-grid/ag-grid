...
export class HeaderCellCtrl extends AbstractHeaderCellCtrl<IHeaderCellComp, AgColumn, ResizeFeature> {
        ...
        const refreshHeaderClasses = () => {
            ...
            classes.forEach((c) => {
                if (oldClasses.has(c)) {
                    ...
                    this.comp.updateCss(c, true);
                }
            });

            // now old set only has classes that were applied last time, but not this time, so remove them
            oldClasses.forEach((c) => this.comp.updateCss(c, false));
        };
    ...
    private setupMovingCss(compBean: BeanStub): void {
        const listener = () => {
            ...
            this.comp.updateCss('ag-header-cell-moving', this.column.isMoving());
        };
    ...
    private setupMenuClass(compBean: BeanStub): void {
        const listener = () => {
            this.comp?.updateCss('ag-column-menu-visible', this.column.isMenuVisible());
        };
    ...
    private setupSortableClass(compBean: BeanStub): void {
        const updateSortableCssClass = () => {
            this.comp.updateCss('ag-header-cell-sortable', !!this.sortable);
        };
    ...
    private setupFilterClass(compBean: BeanStub): void {
        const listener = () => {
            ...
            this.comp.updateCss('ag-header-cell-filtered', isFilterActive);
            this.refreshAria();
        };
    ...
    private setupWrapTextClass() {
        const listener = () => {
            ...
            this.comp.updateCss('ag-header-cell-wrap-text', wrapText);
        };
        listener();
    ...
    private onHeaderHighlightChanged(): void {
        ...
        this.comp.updateCss('ag-header-highlight-before', beforeOn);
        this.comp.updateCss('ag-header-highlight-after', afterOn);
    }
    ...
    private refreshSpanHeaderHeight() {
        ...
        comp.updateCss('ag-header-parent-hidden', isZeroGroupHeight);

        if (!column.isSpanHeaderHeight()) {
            eGui.style.removeProperty('top');
            eGui.style.removeProperty('height');
            comp.updateCss('ag-header-span-height', false);
            comp.updateCss('ag-header-span-total', false);
            return;
        }

        const { numberOfParents, isSpanningTotal } = this.column.getColumnGroupPaddingInfo();

        comp.updateCss('ag-header-span-height', numberOfParents > 0);
        ...
        comp.updateCss('ag-header-span-total', isSpanningTotal);
    ...
    private setActiveHeader(active: boolean): void {
        this.comp.updateCss('ag-header-active', active);
    }
