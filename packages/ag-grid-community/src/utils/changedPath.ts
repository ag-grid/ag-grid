...
export interface IChangedPathFactory {
    /** Creates a new `ChangedRowsPath` (row-level tracking only, no per-column tracking). */
    newRowsPath(): ChangedRowsPath;

    /** Creates a new `ChangedCellsPath` (tracks both rows and which columns changed per row). */
    newCellsPath(): ChangedCellsPath;
