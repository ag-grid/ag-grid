...
> {
    ...
    constructor() {
        super('numberFilter', mapValuesFromBigIntFilterModel, DEFAULT_BIGINT_FILTER_OPTIONS);
    }
    ...
    protected override setElementValue(
        ...
        const { formatter } = this.params;
        const valueToSet = !fromFloatingFilter && formatter ? formatter(value ?? null) : value;
    ...
    protected getValues(position: number): Tuple<bigint> {
        ...
        this.forEachPositionInput(position, (element, index, _elPosition, numberOfInputs) => {
            if (index < numberOfInputs) {
                result.push(processBigIntFilterValue(stringToBigInt(this.params.parser, element.getValue())));
            }
        });
...
function stringToBigInt(parser: IBigIntFilterParams['parser'], value?: string | bigint | null): bigint | null {
    ...
    if (parser) {
        return parser(filterText);
    }

    return filterText == null || filterText.trim() === '-' ? null : BigInt(filterText);
}

function getNormalisedValue(
    parser: IBigIntFilterParams['parser'],
    input: GridInputTextField | GridInputNumberField
): bigint | null {
    return processBigIntFilterValue(stringToBigInt(parser, input.getValue(true)));
}
