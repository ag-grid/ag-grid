...
import type { DomLayoutType, GridOptions, PaginationPanel, PaginationPanelParams } from '../../entities/gridOptions';
            ...
            validate: ({ paginationPanels }) => {
                ...
                if (
                    ...
                    paginationPanels.some((p) => {
                        if (typeof p === 'string') return !validNames.has(p);
                        if (typeof p === 'object' && p !== null)
                            return !validNames.has((p as PaginationPanelParams).type);
                        return true;
                    })
                ) {
                    return "'paginationPanels' expects an array of panel names or config objects: ['pageSize', 'rowSummary', 'pageSummary']";
