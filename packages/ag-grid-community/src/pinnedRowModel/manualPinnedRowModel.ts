...
export class ManualPinnedRowModel extends BeanStub implements IPinnedRowModel {
    ...
    public reset(dispatch = true): void {
        this.forContainers((container) => {
            ...
            // The grand total row is derived from the `grandTotalRow` option, not from manual
            // row-pinning state, so preserve it when resetting manual pins (e.g. restoring grid
            // state that has no rowPinning section). On destroy (dispatch === false) clear it too.
