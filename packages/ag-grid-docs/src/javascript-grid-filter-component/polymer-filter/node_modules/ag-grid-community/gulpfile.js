...
function cleanMain() {
  return gulp.src(['./main.d.ts', 'main.js'], {
    read: false
  }).pipe(clean());
}
...
function tscSrcTask() {
  ...
  return merge([tsResult.dts.pipe(header(dtsHeaderTemplate, {
    pkg: pkg
  })).pipe(gulp.dest('dist/lib')), tsResult.js.pipe(header(headerTemplate, {
    pkg: pkg
...
function tscMainTask() {
  ...
  return merge([tsResult.dts.pipe(replace("\"./", "\"./dist/lib/")).pipe(header(dtsHeaderTemplate, {
    pkg: pkg
  })).pipe(rename("main.d.ts")).pipe(gulp.dest('./')), tsResult.js.pipe(replace("require(\"./", "require(\"./dist/lib/")).pipe(header(headerTemplate, {
    pkg: pkg
  })).pipe(rename("main.js")).pipe(gulp.dest('./'))]);
}
...
function scssTask() {
  ...
  return gulp.src(['src/styles/*.scss', '!src/styles/_*.scss']).pipe(named()).pipe(webpackStream({
    mode: 'production',
    module: {
      rules: [{
        test: /\.scss$/,
  ...
  })).pipe(filter("**/*.css")).pipe(gulp.dest('dist/styles/'));
}