...
export default class AgGridPolymer extends PolymerElement {
  ...
  createPropertyMap() {
    return agGrid.ComponentUtil.ALL_PROPERTIES.concat(agGrid.ComponentUtil.getEventCallbacks()).reduce((map, property) => {
  ...
  initialiseGrid() {
    ...
    if (!this._initialised) {
      ...
      let gridParams = {
        ...
        frameworkFactory: new PolymerFrameworkFactory(new agGrid.BaseFrameworkFactory(), new PolymerComponentFactory()),
        seedBeanInstances: {
  ...
  _gridPropertyChanged(name, newValue) {
    let gridPropertyName = this._propertyMap[name] || name; // for properties set before gridOptions is called
  ...
  globalEventListener(eventType, event) {
    ...
    browserEventNoType.agGridDetails = event; // for when defining events via myGrid.addEventListener('columnresized', function (event) {...
