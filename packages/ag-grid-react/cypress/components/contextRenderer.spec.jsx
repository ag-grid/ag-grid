            ...
            <FontContext.Consumer>
                {(fontWeight) => (
                    <span style={{ fontWeight }}>
                        {' '}
                        {currencySymbol}
                        {this.applyExchangeRate(exchangeRate, this.props.value)}
                    </span>
                )}
            </FontContext.Consumer>
        );
...
const SimpleContextHookExample = () => {
    ...
    return (
        <Context.Provider value={{ store, dispatch }}>
            <div>
                <button onClick={() => populateRowData()} className="populateRowData">
                    Populate Row Data
                </button>
                <button onClick={() => setCurrency('£', 1)} className="setCurrencyToGBP">
                    Set Currency to GBP
                </button>
                <button onClick={() => setCurrency('$', 1.29)} className="setCurrencyToUSD">
                    Set Currency to USD
                </button>
