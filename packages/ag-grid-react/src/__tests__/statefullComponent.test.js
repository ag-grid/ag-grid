...
it("stateful component renders as expected", () => {
    expect(
        component
            ...
            .find(".ag-cell-value")
            .html()
    ).toEqual(`<div class="ag-react-container"><div>Age: 24</div></div>`);
});
...
it("cell should be editable and editor component usable", async () => {
    ...
    await waitForAsyncCondition(() => agGridReact.api.getCellEditorInstances() && agGridReact.api.getCellEditorInstances().length > 0, 5).then(
        () => null,
    ...
    await waitForAsyncCondition(() => agGridReact.api.getCellRendererInstances() && agGridReact.api.getCellRendererInstances().length > 0, 5).then(
        () => null,
    ...
    expect(
        component
            ...
            .find(".ag-cell-value")
            .html()
    ).toEqual(`<div class="ag-react-container"><div>Age: 50</div></div>`);
});
