...
// Then we execute a single traversal to set the level, expanded state, and _leafs.
...
export class TreeGroupStrategy<TData = any> extends BeanStub implements IRowGroupingStrategy<TData> {
    ...
    private initRowChildrenSize(row: RowNode<TData>) {
        let { childrenAfterGroup, _leafs: allLeafs, treeNodeFlags } = row;
        ...
        } else if (oldLen !== len || childrenAfterGroup === allLeafs) {
            if (!childrenAfterGroup || childrenAfterGroup === _EmptyArray || childrenAfterGroup === allLeafs) {
    ...
    private handleCycles(rootNode: RowNode<TData>) {
        ...
        for (const row of rootNode._leafs!) {
    ...
    private loadSelfRef({ rowNode: rootNode, changedRowNodes }: StageExecuteParams<TData>, fullReload: boolean): void {
        const allLeafs: RowNode<TData>[] = rootNode._leafs!;
        const allLeafsLen = allLeafs.length;
        const gos = this.gos;
        ...
        for (let i = 0; i < allLeafsLen; ++i) {
            ...
            if (fullReload || row.treeNodeFlags & FLAG_CHANGED || removals?.has(row.treeParent!)) {
                ...
                if (parentId !== null && parentId !== undefined) {
                    newParent = rowModel.getRowNode(parentId) as RowNode<TData>;
                    if (!newParent) {
