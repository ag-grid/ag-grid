...
export class AgFillHandle extends AbstractSelectionHandle {
    ...
    private clearMarkedPath() {
        this.markedCells.forEach((cell) => {
            ...
            const { comp } = cell;
            comp.updateCss('ag-selection-fill-top', false);
            comp.updateCss('ag-selection-fill-right', false);
            comp.updateCss('ag-selection-fill-bottom', false);
            comp.updateCss('ag-selection-fill-left', false);
        });
            ...
            for (let i = 0; i < colLen; i++) {
                ...
                if (!isInitialRow) {
                    ...
                    if (cell) {
                        ...
                        if (!cellInRange) {
                            cellComp.updateCss('ag-selection-fill-left', i === 0);
                            cellComp.updateCss('ag-selection-fill-right', i === colLen - 1);
                        }
        ...
        do {
            ...
            for (let i = 0; i < colLen; i++) {
                ...
                if (cell) {
                    ...
                    cell.comp.updateCss('ag-selection-fill-bottom', _isSameRow(row, endPosition));
                }
            }
            if (isLastRow) {
                break;
        ...
        colsToMark.forEach((column) => {
            ...
            do {
                ...
                if (cell) {
                    ...
                    cellComp.updateCss('ag-selection-fill-top', _isSameRow(row, rangeStartRow));
                    cellComp.updateCss('ag-selection-fill-bottom', _isSameRow(row, rangeEndRow));
                    if (isMovingLeft) {
                        this.isLeft = true;
                        cellComp.updateCss('ag-selection-fill-left', column === colsToMark[0]);
                    } else {
                        cellComp.updateCss('ag-selection-fill-right', column === _last(colsToMark));
                    }
                }
    ...
    private reduceHorizontal(initialPosition: CellPosition, endPosition: CellPosition) {
        ...
        colsToMark.forEach((column) => {
            ...
            do {
                ...
                if (cell) {
                    this.markedCells.push(cell);
                    cell.comp.updateCss('ag-selection-fill-right', column === colsToMark[0]);
                }
