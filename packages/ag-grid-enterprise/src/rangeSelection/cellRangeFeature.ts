...
export class CellRangeFeature implements ICellRangeFeature {
    ...
    public onCellSelectionChanged(): void {
        ...
        cellComp.updateCss(CSS_CELL_RANGE_SELECTED, rangeCount !== 0);
        cellComp.updateCss(`${CSS_CELL_RANGE_SELECTED}-1`, rangeCount === 1);
        cellComp.updateCss(`${CSS_CELL_RANGE_SELECTED}-2`, rangeCount === 2);
        cellComp.updateCss(`${CSS_CELL_RANGE_SELECTED}-3`, rangeCount === 3);
        cellComp.updateCss(`${CSS_CELL_RANGE_SELECTED}-4`, rangeCount >= 4);
        cellComp.updateCss(CSS_CELL_RANGE_CHART, hasChartRange);

        _setAriaSelected(eGui, rangeCount > 0 ? true : undefined);
        cellComp.updateCss(CSS_CELL_RANGE_SINGLE_CELL, this.isSingleCell());
    ...
    private updateRangeBorders(): void {
        ...
        const cellComp = this.cellComp;
        cellComp.updateCss(CSS_CELL_RANGE_TOP, isTop);
        cellComp.updateCss(CSS_CELL_RANGE_RIGHT, isRight);
        cellComp.updateCss(CSS_CELL_RANGE_BOTTOM, isBottom);
        cellComp.updateCss(CSS_CELL_RANGE_LEFT, isLeft);
    }
    ...
    public refreshHandle(): void {
        ...
        this.cellComp.updateCss(CSS_CELL_RANGE_HANDLE, !!this.selectionHandle);
    }
    ...
    private shouldHaveSelectionHandle(): boolean {
        ...
        if (this.hasChartRange) {
            ...
            this.cellComp.updateCss(CSS_CELL_RANGE_CHART_CATEGORY, isCategoryCell);
