...
export class AutoColService extends BeanStub implements NamedBean, IAutoColService {
    ...
    public createColumns(source: ColumnEventType): boolean {
        ...
        if (!noAutoCols) {
            ...
            if (_areColIdsEqual(list, this.columns)) {
                // Columns unchanged — just update colDefs in case header etc. changed
            ...
            beans.context.destroyBeans(this.columns);
            this.columns = list;
            return true;
        }
    ...
    public updateColumns(event: PropertyValueChangedEvent<'autoGroupColumnDef'>) {
        ...
        this.columns.forEach((col, index) => this.updateOneAutoCol(col, index, source));
    }
