 ...
 * @param hi
 */
export function bisectLeft(list, x, comparator, lo = 0, hi = list.length) {
    while (lo < hi) {
        const mid = (lo + hi) >>> 1;
...
export function bisectRight(list, x, comparator, lo = 0, hi = list.length) {
    while (lo < hi) {
        const mid = (lo + hi) >>> 1;
        if (comparator(list[mid], x) > 0) { // list[mid] > x
            hi = mid;
 ...
 * @param hi
 */
export function complexBisectLeft(list, x, map, lo = 0, hi = list.length) {
...
export function complexBisectRight(list, x, map, lo = 0, hi = list.length) {
    const comparator = ascendingComparator(map);
    while (lo < hi) {
        const mid = (lo + hi) >>> 1;
        if (comparator(list[mid], x) < 0) {
