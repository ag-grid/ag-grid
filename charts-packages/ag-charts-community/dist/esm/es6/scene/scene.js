...
export class Scene {
    constructor(opts) {
        ...
        const { document = window.document, mode = windowValue('agChartsSceneRenderModel') || 'adv-composite', width, height, } = opts;
    ...
    buildTree(node) {
        ...
        return Object.assign({ name,
            node, dirty: RedrawType[node.dirty] }, node.children.map((c) => this.buildTree(c))
    ...
    buildDirtyTree(node) {
        ...
        return {
            dirtyTree: Object.assign({ name,
                node, dirty: RedrawType[node.dirty] }, childrenDirtyTree.map((c) => c.dirtyTree)
