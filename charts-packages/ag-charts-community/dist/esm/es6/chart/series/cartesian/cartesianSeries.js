...
export class CartesianSeries extends Series {
    ...
    validateXYData(xKey, yKey, data, xAxis, yAxis, xData, yData, yDepth = 1) {
        ...
        if (isContinuousX && !hasNumber(xData)) {
            doOnce(() => console.warn(`AG Charts - The number axis has no numeric data supplied for xKey: [${xKey}].`), 'series has no numeric data on number axis - ' + xKey);
            validationResult = false;
        }
        if (isContinuousY && !hasNumber(yData, 0, yDepth - 1)) {
            doOnce(() => console.warn(`AG Charts - The number axis has no numeric data supplied for yKey: [${yKey}].`), 'series has no numeric data on number axis - ' + yKey);
            validationResult = false;
        }
