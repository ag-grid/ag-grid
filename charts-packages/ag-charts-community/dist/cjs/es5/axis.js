...
var Axis = /** @class */ (function () {
    function Axis(scale) {
        ...
        this.axisGroup = new group_1.Group({ name: this.id + "-axis", layer: true, zIndex: 50 });
        this.tickLineGroup = this.axisGroup.appendChild(new group_1.Group());
        this.titleGroup = this.axisGroup.appendChild(new group_1.Group());
        this.tickLineGroupSelection = selection_1.Selection.select(this.tickLineGroup).selectAll();
        this.lineNode = this.tickLineGroup.appendChild(new line_1.Line());
        this.gridlineGroup = new group_1.Group({ name: this.id + "-gridline", layer: true, zIndex: 0 });
    ...
    Axis.prototype.onLabelFormatChange = function (format) {
        if (format && this.scale && this.scale.tickFormat) {
            try {
                ...
                function_1.doOnce(function () { return console.warn("AG Charts - the axis label format string " + format + " is invalid. No formatting will be applied"); }, "invalid axis label format string " + format);
            }
        }
        else {
            this.labelFormatter = undefined;
    ...
    Axis.prototype.update = function () {
        ...
        var labelSelection = tickLineGroupSelection.selectByClass(text_1.Text)
        ...
        if (totalLabelLength > availableRange) {
            var isContinuous_1 = scale instanceof continuousScale_1.ContinuousScale;
