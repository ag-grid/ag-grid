...
function prepareOptions(newOptions, fallbackOptions) {
    var options = fallbackOptions == null ? newOptions : json_1.jsonMerge(fallbackOptions, newOptions);
    ...
    var defaultOptions = isAgCartesianChartOptions(options) ? defaults_1.DEFAULT_CARTESIAN_CHART_OPTIONS :
    ...
    var defaultSeriesType = isAgCartesianChartOptions(options) ? 'line' :
        isAgHierarchyChartOptions(options) ? 'treemap' :
            isAgPolarChartOptions(options) ? 'pie' :
                'line';
    var defaultOverrides = type === 'bar' ? defaults_1.DEFAULT_BAR_CHART_OVERRIDES :
...
function prepareMainOptions(defaultOptions, defaultOverrides, options) {
    ...
    var mergedOptions = json_1.jsonMerge(defaultOptions, defaultOverrides, cleanedTheme, options);
...
function prepareAxis(input) {
    ...
    return json_1.jsonMerge.apply(void 0, __spread(defaults, [input, removeOptions]));
}
