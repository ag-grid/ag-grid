...
var TreemapSeries = /** @class */ (function (_super) {
    ...
    TreemapSeries.prototype.updateNodes = function () {
        ...
        var updateNodeFn = function (text, datum, index, highlighted) {
            ...
            var _a = labelMeta[index], label = _a.label, textBaseline = _a.nodeBaseline;
        ...
        this.groupSelection.selectByTag(TextNodeTag.Name)
            .each(function (text, datum, index) { return updateNodeFn(text, datum, index, false); });
        this.highlightSelection.selectByTag(TextNodeTag.Name)
        ...
        var updateValueFn = function (text, datum, index, highlighted) {
            var _a = labelMeta[index], textBaseline = _a.valueBaseline, valueText = _a.valueText;
        ...
        this.groupSelection.selectByTag(TextNodeTag.Value)
            .each(function (text, datum, index) { return updateValueFn(text, datum, index, false); });
        this.highlightSelection.selectByTag(TextNodeTag.Value)
            .each(function (text, datum, index) {
            var isDatumHighlighted = datum === highlightedDatum;
            text.visible = isDatumHighlighted;
            if (text.visible) {
    ...
    TreemapSeries.prototype.buildLabelMeta = function (data) {
        ...
        try {
            for (var data_1 = __values(data), data_1_1 = data_1.next(); !data_1_1.done; data_1_1 = data_1.next()) {
                ...
                var hasNode = isLeaf && !!nodeBBox
                    && nodeBBox.width <= innerNodeWidth
                    && nodeBBox.height * 2 + 8 <= innerNodeHeight;
                var valueText = typeof value === 'number' && isFinite(value)
                ...
                var nodeBaseline = hasValue ? 'bottom' :
                    isLeaf ? 'middle' :
                        (hasTitle ? 'top' : 'middle');
                labelMeta[index++] = {
                    label: label,
