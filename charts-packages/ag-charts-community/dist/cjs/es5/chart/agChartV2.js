...
var AgChartV2 = /** @class */ (function () {
    ...
    AgChartV2.update = function (chart, userOptions) {
        ...
        var deltaOptions = json_1.jsonDiff(chart.options, mergedOptions, { stringify: ['data'] });
    ...
    try {
        for (var _b = __values(options || []), _c = _b.next(); !_c.done; _c = _b.next()) {
            ...
            switch (seriesOptions.type) {
                case 'area':
                    series.push(applySeriesValues(new areaSeries_1.AreaSeries(), seriesOptions, { path: path, skip: skip }));
                    break;
                case 'bar':
                case 'column':
                    series.push(applySeriesValues(new barSeries_1.BarSeries(), seriesOptions, { path: path, skip: skip }));
                    break;
                case 'histogram':
                    series.push(applySeriesValues(new histogramSeries_1.HistogramSeries(), seriesOptions, { path: path, skip: skip }));
                    break;
                case 'line':
                    series.push(applySeriesValues(new lineSeries_1.LineSeries(), seriesOptions, { path: path, skip: skip }));
                    break;
                case 'scatter':
                    series.push(applySeriesValues(new scatterSeries_1.ScatterSeries(), seriesOptions, { path: path, skip: skip }));
                    break;
                case 'pie':
                    series.push(applySeriesValues(new pieSeries_1.PieSeries(), seriesOptions, { path: path, skip: skip }));
                    break;
                case 'treemap':
                    series.push(applySeriesValues(new treemapSeries_1.TreemapSeries(), seriesOptions, { path: path, skip: skip }));
                    break;
                default:
    ...
    try {
        for (var _b = __values(options || []), _c = _b.next(); !_c.done; _c = _b.next()) {
            ...
            switch (axisOptions.type) {
                case 'number':
                    axes.push(applyAxisValues(new numberAxis_1.NumberAxis(), axisOptions, { path: path }));
                    break;
                case logAxis_1.LogAxis.type:
                    axes.push(applyAxisValues(new logAxis_1.LogAxis(), axisOptions, { path: path }));
                    break;
                case categoryAxis_1.CategoryAxis.type:
                    axes.push(applyAxisValues(new categoryAxis_1.CategoryAxis(), axisOptions, { path: path }));
                    break;
                case groupedCategoryAxis_1.GroupedCategoryAxis.type:
                    axes.push(applyAxisValues(new groupedCategoryAxis_1.GroupedCategoryAxis(), axisOptions, { path: path }));
                    break;
                case timeAxis_1.TimeAxis.type:
                    axes.push(applyAxisValues(new timeAxis_1.TimeAxis(), axisOptions, { path: path }));
                    break;
                default:
