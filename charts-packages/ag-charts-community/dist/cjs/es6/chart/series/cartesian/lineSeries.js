    ...
    processData() {
        ...
        for (let i = 0, n = data.length; i < n; i++) {
            ...
            if (isContinuousX) {
                ...
                // i.e. category axis
                xData.push(value_1.isDiscrete(x) ? x : String(x));
            }
            if (isContinuousY) {
                yData.push(y);
            }
            else {
                yData.push(value_1.isDiscrete(y) ? y : String(y));
            }
        }
        this.xDomain = isContinuousX ? this.fixNumericExtent(array_1.extent(xData, value_1.isContinuous), 'x') : xData;
        this.yDomain = isContinuousY ? this.fixNumericExtent(array_1.extent(yData, value_1.isContinuous), 'y') : yData;
        return true;
    }
    getDomain(direction) {
