...
class CartesianSeries extends series_1.Series {
    ...
    checkDatum(value, isContinuousScale) {
        if (isContinuousScale && value_1.isContinuous(value)) {
            return value;
        }
        else if (!isContinuousScale) {
            if (!value_1.isDiscrete(value)) {
                return String(value);
            }
            return value;
        }
        ...
        while (contextNodeData.length > subGroups.length) {
            ...
            const markerGroup = features.includes('markers') ?
                new group_1.Group({
                    ...
                    layer: true,
                    zIndex: series_1.Series.SERIES_MARKER_LAYER_ZINDEX,
                }) :
                undefined;
    ...
    updateNodes(seriesHighlighted, anySeriesItemEnabled) {
        ...
        this.subGroups.forEach((subGroup, seriesIdx) => {
            ...
            if (markerGroup) {
                ...
                markerGroup.zIndex = group.zIndex + (series_1.Series.SERIES_MARKER_LAYER_ZINDEX - series_1.Series.SERIES_LAYER_ZINDEX);
