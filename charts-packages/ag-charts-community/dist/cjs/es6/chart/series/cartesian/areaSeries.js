    ...
    processData() {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (const datum of data) {
                // X datum
                if (!(xKey in datum)) {
                    function_1.doOnce(() => console.warn(`AG Charts - The key '${xKey}' was not found in the data: `, datum), `${xKey} not found in data`);
                    continue;
                }
            ...
            if (missingYKeys.size > 0) {
                ...
                function_1.doOnce(() => console.log(`AG Charts - yKeys ${missingYKeysString} were not found in the data.`), `${missingYKeysString} not found in data.`);
            }
            const xyValid = this.validateXYData(this.xKey, this.yKeys.join(', '), data, xAxis, yAxis, xData.map((x) => x.xDatum), yData, 2);
            if (!xyValid) {
                this.xData = [];
                this.yData = [];
                this.yDomain = [];
            ...
            const isLogAxis = yAxis instanceof logAxis_1.LogAxis;
