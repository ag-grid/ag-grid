...
const utcDay_1 = require("../utcDay");
const utcYear_1 = require("../utcYear");
const utcWeek_1 = require("../utcWeek");
 ...
 * @param s
 */
exports.requote = (s) => s.replace(requoteRe, '\\$&'); // $& - matched substring
/**
 * Returns a RegExp that matches any string that starts with any of the given names (case insensitive).
 * @param names
 */
exports.formatRe = (names) => new RegExp('^(?:' + names.map(exports.requote).join('|') + ')', 'i');
...
function formatLocale(timeLocale) {
    ...
    const periodRe = exports.formatRe(lPeriods);
    const periodLookup = formatLookup(lPeriods);
    const weekdayRe = exports.formatRe(lWeekdays);
    const weekdayLookup = formatLookup(lWeekdays);
    const shortWeekdayRe = exports.formatRe(lShortWeekdays);
    const shortWeekdayLookup = formatLookup(lShortWeekdays);
    const monthRe = exports.formatRe(lMonths);
    const monthLookup = formatLookup(lMonths);
    const shortMonthRe = exports.formatRe(lShortMonths);
    ...
    function formatWeekNumberISO(date, fill) {
        ...
        const yearStart = year_1.default.floor(date);
    ...
    function formatUTCWeekNumberISO(date, fill) {
        ...
        const yearStart = utcYear_1.default.floor(date);
