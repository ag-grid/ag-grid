...
function getMapping(path) {
    ...
    var value = chartMappings_1.default;
...
function update(component, options, path) {
            ...
            if (skipKeys.indexOf(key) < 0) {
                ...
                if (meta.setAsIs && meta.setAsIs.indexOf(key) >= 0) {
                    ...
                    if (Array.isArray(oldValue) && Array.isArray(value)) {
                        if (path in chartMappings_1.default) { // component is a chart
                            if (key === 'series') {
                                var chart = component;
                                var configs = value;
    ...
    if (path in chartMappings_1.default) { // top-level component (chart)
...
function provideDefaultChartType(options) {
    ...
    if (!options.type) {
        ...
        if (series && series.type) {
            outerLoop: for (var chartType in chartMappings_1.default) {
                for (var seriesType in chartMappings_1.default[chartType].series) {
