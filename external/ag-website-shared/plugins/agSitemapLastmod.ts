...
/**
 * Build a map of source path → last-modified Date.
 *
 * Tries a pre-built JSON cache first (.astro/cache/sitemap-lastmod.json),
 * then falls back to a live `git log` traversal. The cache is generated by
 * external/ag-website-shared/scripts/generateSitemapLastmodCache.mjs in a
 * nightly CI job that uses a full (non-shallow) clone.
 */
type DateMapResult = { map: Map<string, Date>; source: 'cache' | 'git' | 'none' };

async function buildDateMap(cwd: string): Promise<DateMapResult> {
    const cacheFile = path.join(cwd, '.astro', 'cache', 'sitemap-lastmod.json');
    try {
        const raw = JSON.parse(await readFile(cacheFile, 'utf8')) as Record<string, string>;
        const map = new Map<string, Date>();
        for (const [rel, iso] of Object.entries(raw)) {
            const d = new Date(iso);
            if (!isNaN(d.getTime())) map.set(path.resolve(cwd, rel), d);
        }
        if (map.size > 0) return { map, source: 'cache' };
    } catch {
        // Cache absent or unreadable — fall through to git log
    }
    ...
    return { map, source: map.size > 0 ? 'git' : 'none' };
}
    ...
    return {
        ...
        hooks: {
            ...
            'astro:build:done': async ({ dir, logger }) => {
                ...
                const { map: dateMap, source } = await buildDateMap(cwd);
                if (source === 'cache') {
                    logger.info(`Using pre-built lastmod cache (${dateMap.size} entries)`);
                } else if (source === 'git') {
                    logger.info(`Built lastmod map from git history (${dateMap.size} entries)`);
                } else {
                    logger.warn('Could not read git history or cache — falling back to filesystem modification times');
                }
